...
Schema_Version: 1.1 (Phase 3-1 - bidirectional schema sync with LA12X_v5.1 + Total_Power_Capability correction)
...
본 파일은 SKILL v1.7의 "완벽한 양방향 스키마 동기화" 규칙에 따라 LA12X_v5.1.md와 Key 목록을 100% 일치시킨 v1.1 개정본이다. LA12X 대비 신규로 발견된 속성(Key)은 [신규] 표시로, LA12X에만 있던 기존 속성 중 LA7.16에 역주입된 Key는 [양방향 동기화] 표시로 구분했다. 또한 v1.0의 `Total_Power_Capability` 오기재를 정정했다(상세는 amplification/power_supply 섹션 각주 참조).
