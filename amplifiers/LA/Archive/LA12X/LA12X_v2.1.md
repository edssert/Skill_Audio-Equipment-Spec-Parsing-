...
Schema_Version: 2.1 (재분류됨, 구 v3.0 - Phase 2 - integrity fix per skill update v0.3: resistance notation standardization, no unsourced modifiers)

버전 재분류 안내 (SKILL v1.8 시맨틱 버저닝 소급 적용): 이 파일은 원래 v3.0으로 발행되었으나, v2.0에서 도입된 각주 시스템 전면 개편 이후의 변경(Amplification_Class 각주 오류 수정)은 문서 구조 변경이 아닌 단순 수정이므로 Minor 버전(v2.1)으로 재분류되었다. 이하 "버전 변경 이력" 절의 서술은 발행 당시 원문 그대로 보존한다.
...
| Rated_Power_Per_Ch_2R7 [2] | 3300 | W |
| Rated_Power_Per_Ch_4R [2] | 2600 | W |
| Rated_Power_Per_Ch_8R [2] | 1400 | W |
...
**측정 조건**: 출력 정격은 CEA-2006/490A 기준, 1 kHz sine burst, THD < 1%, 전 채널 구동 조건에서 측정. Amplification_Gain, Output_Delay_Range는 별도 조건 없이 사양값. 부하 임피던스 표기는 원문에 이미 소수점 형식(2.7/4/8 ohms)으로 되어 있어 별도 변환 불필요 — 키 이름의 `2R7`/`4R`/`8R`은 관용적 축약 표기(파싱 규칙상 저항값 정제 대상은 원문이 `2R7` 식 유럽식 표기일 때만 해당하며, 본 소스에서는 해당 없음).
