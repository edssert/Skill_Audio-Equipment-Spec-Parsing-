...
Schema_Version: 2.3 ([신규] 각주 태그 전면 제거 — 태그가 추가 시점 이후 버전이 계속 올라가면서 더 이상 신규가 아닌데도 방치되어 있던 문제 정리, 구조/데이터 변경 없는 Minor 버전)
최종 작업 일시: 2026-07-18
...
**[15] AES67_Support 확정적 비존재**: OM 89페이지 전량 텍스트 스캔 및 AE 전문 검색 결과 "AES67"이라는 용어가 단 한 번도 등장하지 않는다 — LA2Xi(v4.0에서 신규 추가)와 달리 LA4X는 AES67을 전혀 지원하지 않는 것으로 확정 판단했다(SKILL v1.9 "확정된 비존재" 원칙 적용, 전문 검색 근거 명시).
...
**섹션 전체 비적용**: LA4X는 Dante가 아닌 Milan-AVB(L-Acoustics 고유 인증) 오디오 네트워크를 사용한다. 이 섹션은 5D(d&b audiotechnik Dante 설치용 라인) 파싱 중 신설되었으며, Milan-AVB 기준으로 설계된 input_avb 섹션과 별도로 Dante 프로토콜 전용 섹션으로 분리되었다. LA4X의 기존 소스에는 이 개념에 대응하는 서술이 없어 전량 null로 소급 반영한다.
...
**[구조 개편] Fault_Contact_Type**: 5D(d&b audiotechnik) 파싱 중 신설된 Key(FAULT 릴레이 접점, NO/C/NC). LA4X의 기존 소스에는 이 개념에 대응하는 서술이 없어 null로 소급 반영한다.
...
**[19] Settings_Protection — 구체적 메커니즘 확보**: LA1.16i/LA2Xi/LA7.16에서는 null이었던 항목인데, LA4X OM은 "Settings protection" 절(p.63)에서 상세히 서술한다 — LA Network Manager에서만 활성화/비활성화 가능하며, 활성화 시 일부 설정은 완전 잠금(locked), 일부는 4자리 PIN 코드로 일시 우회 가능(PIN code protected). 잠금 대상: quick access gain, DELETE PRESET, RESET TO FACTORY DEFAULT SETTINGS. PIN 보호 대상: LOAD PRESET FACTORY, STORE PRESET, PRESET PARAMETERS, CLEAR GROUP PARAMS, NETWORK ADDRESS. 이 신규 정보는 향후 다른 제품 재검토 시 참고할 만하다(다른 제품에서 null 처리된 것이 실제 비지원이 아니라 단순히 소스에 서술이 없었을 가능성 시사).

**[20] GPIO_Count 확정적 비존재**: LA2Xi(4개), LA1.16i(3개)와 달리 LA4X는 OM 89페이지 전량 검색 결과 "GPIO"라는 용어가 전혀 등장하지 않는다 — LA4X에는 GPIO 기능 자체가 없는 것으로 확정 판단했다(SKILL v1.9 원칙 적용).

**[21] DSP_Backup_Power_Input 확정적 비존재**: LA1.16i/LA2Xi에는 24V DC 백업 전원 입력이 있었으나, LA4X의 OM/AE 어디에도 24V DC 관련 서술이 전혀 없다 — 전량 검색 결과 확정적 비지원으로 판단.
...
**[27] Web_Remote_Interface**: 10D(d&b audiotechnik 차세대 설치용 라인) 파싱 중 발견된 신규 Key — 별도 소프트웨어(LA Network Manager) 없이 표준 웹 브라우저로 기기에 직접 접속하는 브라우저 기반 내장 웹 인터페이스 개념. LA4X의 기존 소스에는 이 개념에 대응하는 서술이 확인되지 않아 null(미확인)로 소급 반영한다.
...
이번 소스 집합에서 정의되지 않아 null 처리된 항목 (총 46건 — 본 v2.2 편집 중 grep(`\| null \|`) 재검증 결과 이전 버전의 서두 표기(v2.1 "50건")가 실제 테이블 대비 과다 기재된 오기였음이 확인되어 바로잡음; v2.0의 42건에서 5D 파싱 중 발견된 신규 스키마 확장분 — input_dante 섹션 7건 + Fault_Contact_Type 1건 — 및 10D 파싱 중 발견된 신규 Key 1건(Web_Remote_Interface) 소급 반영 포함):
...
**비적용 — 5D 스키마 동기화 반영(Dante 미사용, 총 7건)**: Dante_RX_Channels, Dante_Sampling_Rates, Dante_SRC, Dante_Redundancy, Dante_Network_Flows, Dante_Latency, Dante_IP_Architecture — 5D(d&b audiotechnik Dante 설치용 라인) 파싱 중 신설된 input_dante 섹션 전체가 양방향 동기화 규칙에 따라 LA4X에도 추가되었다. LA4X는 Milan-AVB 오디오 네트워크를 사용하며 Dante를 사용하지 않아 전량 비적용(null).

**미확인 — 5D 스키마 동기화 반영(총 1건)**: Fault_Contact_Type(connectivity) — 5D 파싱 중 신설된 Key(FAULT 릴레이 접점, NO/C/NC). LA4X의 기존 소스(OM/AE)에는 이 개념에 대응하는 서술이 없어 null로 소급 반영.

**미확인 — 10D 스키마 동기화 반영(총 1건)**: Web_Remote_Interface(control_monitoring) — 10D 파싱 중 신설된 브라우저 기반 내장 웹 인터페이스 Key. LA4X의 기존 소스(OM/AE)에는 이 개념에 대응하는 서술이 없어 null로 소급 반영.
...
v2.0 to v2.1: (5D 스키마 동기화, Minor — null-only 추가) 신규 제품 5D(d&b audiotechnik, Dante 설치용 라인, 5D_v1.0.md) 파싱 중 기존 스키마 어디에도 대응 개념이 없는 두 가지 신규 확장이 발견되어, 양방향 스키마 동기화 규칙(SKILL "완벽한 양방향 스키마 동기화")에 따라 LA4X에도 반영했다. (1) **input_dante 신규 섹션(7개 Key)**: Dante_RX_Channels, Dante_Sampling_Rates, Dante_SRC, Dante_Redundancy, Dante_Network_Flows, Dante_Latency, Dante_IP_Architecture — Milan-AVB 기준으로 설계된 input_avb 섹션과는 별개의 오디오오버이더넷 프로토콜(Audinate Dante)을 위해 5D 파싱 중 신설된 섹션이며, input_avb 섹션 바로 뒤, network 섹션 바로 앞에 삽입했다(5D_v1.0.md와 동일한 섹션 순서). LA4X는 Dante가 아닌 Milan-AVB를 사용하므로 7개 Key 전량 null(비적용)로 반영했다. (2) **connectivity 섹션의 Fault_Contact_Type 신규 Key**: 원격 감시용 FAULT 릴레이 접점(NO/C/NC)을 나타내는 Key로, 기존 스키마 어디에도 대응 Key가 없어 5D 파싱 중 신설되었다. connectivity 섹션의 실제 마지막 행(Output_Mode_Selectable [17a]) 바로 다음에 새 마지막 행으로 추가했으며(PBTL_Activation_Method는 표 상 마지막 행이 아니었음), 각주 태그는 이 섹션의 기존 태그([16], [17], [17a])와 충돌하지 않도록, 그리고 이 파일 전체에서 이미 사용 중인 최고 번호([25], protection_summary 섹션)와도 겹치지 않도록 [26]을 신규 부여했다. LA4X는 대응 서술이 없어 null(미확인)로 반영했다. Null Report를 42건에서 50건으로 재계산(input_dante 7건을 "비적용 — 5D 스키마 동기화 반영" 그룹으로, Fault_Contact_Type 1건을 "미확인 — 5D 스키마 동기화 반영" 그룹으로 별도 신설해 추가, 총 +8건) — grep으로 테이블 내 "| null" 셀 실개수(50건)를 직접 검증했다. 이번 버전은 순수 null 추가이며 LA4X의 기존 실측 Value/각주/근거는 전혀 변경하지 않았다 — Surgical Versioning Protocol에 따라 LA4X_v2.0.md를 무편집 복제 후 Archive/Archive_Data/LA4X/로 이관하고, 신규 파일(v2.1)에서만 편집을 수행했다.

v2.1 to v2.2: (10D 스키마 동기화, Minor — null-only 추가) d&b 차세대 설치용 라인 신규 제품 10D(10D_v1.0.md) 파싱 중 발견된 신규 Key `Web_Remote_Interface`를 control_monitoring 섹션의 Signal_Path_Management[21a] 다음 마지막 행에 null로 추가했다 — 별도 소프트웨어(LA Network Manager) 없이 표준 웹 브라우저로 기기에 직접 접속하는 브라우저 기반 내장 웹 인터페이스 개념이며, 각주 태그는 이 섹션의 기존 태그([21a])와 충돌하지 않도록 [27]을 신규 부여했다(connectivity 섹션의 [26] Fault_Contact_Type과도 겹치지 않음). LA4X의 기존 소스(OM/AE)에는 대응 서술이 없어 null(미확인)로 반영했다. 이번 편집 중 grep(`\| null \|`) 재검증 결과 실제 테이블의 null 셀 개수가 이전 버전의 서두 표기(50건)보다 적은 46건(Web_Remote_Interface 추가 반영 후 기준)임이 확인되어, Null Report 서두의 총계 표기를 46건으로 바로잡았다(과거 버전의 실제 Value/Key에는 변경 없음, 서술 오기만 정정). 이번 버전은 순수 null 추가이며 LA4X의 기존 실측 Value/각주/근거는 전혀 변경하지 않았다 — Surgical Versioning Protocol에 따라 LA4X_v2.1.md를 무편집 복제 후 Archive/Archive_Data/LA4X/로 이관하고, 신규 파일(v2.2)에서만 편집을 수행했다.

v2.2 to v2.3: (표기 정리, Minor) 프로젝트 전체 재검토에서 [신규] 각주 태그가 추가 시점 이후 버전이 계속 올라가면서 더 이상 "신규"가 아닌데도 방치되어 있던 문제가 발견되어, 각주 번호(예: [27])는 유지한 채 [신규] 텍스트만 전면 제거했다. 데이터/Key/Value/Unit/Null Report는 전혀 변경되지 않았다.
