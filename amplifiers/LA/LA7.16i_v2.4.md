...
Schema_Version: 2.4 (구조 개편 v2.0에서 위치만 이동되고 "재조회하지 않음"으로 보수 처리되었던 18개 Key를 LA7.16i 자신의 OM v5.0/AE 원본에서 실제로 재검색 — Network_IP_Default 1건 실값 확보, AVB_Device_Type 1건 기존 Key와 교차 확인 후 값 기재, 나머지 16건은 전량 재검색 완료 후 미확인 유지로 각주 갱신)
최종 작업 일시: 2026-07-18
...
**[4a][v2.4 재검증] Mains_Current_Limiter_Range**: OM p.63 "Specifications > Protection" 절을 "current limiter"/"inrush" 키워드로 재검색한 결과 "overcurrent (fuse protection, and inrush current protection)"라는 보호 기능 존재 서술(이미 protection_summary 섹션의 Protection_Mains_PSU에 반영됨)은 확인되나, D80의 "95 to 50 % of 16/30 A"처럼 전류 제한기의 조정 가능 범위(%)를 수치로 제공하는 스펙 항목은 OM/AE 어디에도 없다. 미확인(null)으로 유지한다.
...
**[5a][v2.4 재검증] SN_Ratio_Analog_Input/SN_Ratio_Digital_Input**: OM/AE의 "General" 스펙 항목(OM p.62, AE "Technical requirements > Amplification")을 "signal to noise", "SNR", "S/N" 키워드로 재검색했으나, LA7.16i의 소스는 Output_Dynamic_Range(> 119 dB, digital input 기준)와 Noise_Level(< -79 dBV)만 제공하며 입력 경로(analog/digital)별로 별도 구분된 S/N ratio 수치는 OM/AE 어디에도 존재하지 않는다. 미확인(null)으로 유지한다.

**[5b][v2.4 재검증] Output_Noise_Dynamic_Range_Detail**: 동일하게 OM/AE 전량을 재검색했으나, LA7.16i는 위 Output_Dynamic_Range/Noise_Level 두 항목 외에 별도의 절대 노이즈 전압(예: dBV/dBu 단위의 unweighted 상세치) 기반 세부 구조를 제공하지 않는다. 미확인(null)으로 유지한다.
...
**[6a][v2.4 재검증] System_Start_Up_Time/LoadMatch_Max_Cable_Length/Load_Monitoring_System_Check**: OM 전량을 "start up"/"startup"/"boot"/"cable length"/"LoadMatch"/"load match"/"load monitoring" 키워드로 재검색했다. System_Start_Up_Time: OM p.19 "CHK - Normal start-up sequence" 절은 "Check that all the LEDs light up in orange during the start-up sequence" 등 정성적 점검 절차만 서술할 뿐 부팅 소요 시간(초)을 수치로 제공하지 않는다. LoadMatch_Max_Cable_Length/Load_Monitoring_System_Check: LA7.16i는 d&b LoadMatch(스피커 케이블 임피던스 보상 및 부하 상태 자가진단) 방식의 스피커 라인 모니터링 개념 자체가 OM/AE 어디에도 서술되어 있지 않다(보호 회로는 protection_summary 섹션의 overcurrent/DC/short circuit/over temperature 감지로 한정됨). 3개 Key 모두 미확인(null)으로 유지한다.

**[6b][v2.4 재검증] Time_To_Tone/Energy_Saving_Detail/AmpPresets_Detail**: OM 전량을 "time to tone"/"energy sav"/"eco mode"/"standby"/"amp preset" 키워드로 재검색했다. Time_To_Tone: standby 진입/복귀는 OM p.36 "Operation > Powering on/off"에 서술되나("Use LA Network Manager to set the amplified controller to standby or back to operating mode") 상태 전환별 소요 시간(초) 수치는 제공되지 않는다. Energy_Saving_Detail: Power_Consumption_Standby(18W, 이미 표에 기재)는 확인되나, 이는 사용자가 LA Network Manager로 수동 전환하는 단순 대기모드일 뿐 d90_specific 유래 개념(자동 EcoMode 카운트다운, AutoStandby/AutoWakeup 임계값 등 자동화된 절전 로직)에 대응하는 서술은 없다. AmpPresets_Detail: OM p.13 "flash memory for preset storage and management"/AE "Built-in library with factory presets for all manufacturers' loudspeakers and 10 user memories"로 프리셋 시스템 존재 자체는 확인되나(참고로 Preset_Memory_User/Factory Key는 이번 재검증 범위 밖이므로 값을 채우지 않음), d&b 유래 AmpPresets_Detail 개념처럼 슬롯을 용도별(User/Alarm/Backup 등)로 세분화하여 접근 권한까지 명시하는 구조는 LA7.16i 소스에 없다. 3개 Key 모두 미확인(null)으로 유지한다.
...
**[7a][v2.4 재검증] CMRR_Analog_Input**: v2.0 구조 개편 당시에는 LA7.16i의 OM/AE를 재조회하지 않고 위치만 이동했으나, 이번 v2.4에서 OM 전량(69p 텍스트 레이어) 및 AE 전량을 "CMRR", "common mode", "common-mode", "rejection" 키워드로 재검색했다. Analog input 관련 서술(OM p.15 "Analog", p.64 spec 표)에는 Input_Impedance(22 kOhm)/Max_Input_Level(22 dBu)/ADC dynamic range만 명시되어 있을 뿐 CMRR 수치나 "common-mode rejection" 문구 자체가 OM/AE 어디에도 존재하지 않는다. 미확인(null)으로 유지한다.
...
**[1][v2.4 재검증] Digital_Input_Impedance**: OM p.31 "Audio and network cabling > Cables for AES/EBU digital audio" 절은 "AES3 specifies that the nominal characteristic impedance of cables used for AES/EBU digital audio transmission shall be 110 Ω ± 20%..."라고 서술하나, 이는 AES3 규격이 요구하는 **케이블의** 특성 임피던스를 설명하는 일반 규격 안내 문구이며, LA7.16i 자신의 AES/EBU **수신단(리시버) 입력 임피던스**가 110Ω이라고 명시적으로 확정하는 문장은 아니다(디지털 입력 스펙 표 OM p.64에도 이 항목 자체가 없음). 임의 추론으로 110Ω을 채택하지 않고, AE에도 대응 서술이 없음을 확인하여 미확인(null)으로 유지한다.
...
| AVB_Device_Type [8a] | Bridge and Listener | - |
...
**[8a][v2.4 재검증] AVB_Device_Type**: OM p.64 "Specifications > Milan-AVB" 스펙 표의 "Featured AVB entities: MILAN™-compliant, Avnu™-compliant Milan-AVB Bridge and Listener" 문구를 근거로 값을 채택했다. 이 문구는 위 AVB_Certification Key의 Value("MILAN and Avnu certified (Bridge and Listener)")를 구성할 때 이미 인용한 것과 동일한 출처/문장으로, D90처럼 AVB_Certification이 null이면서 AVB_Device_Type만 별도 값("Endstation")을 갖는 구조와 달리 LA7.16i는 두 Key가 사실상 같은 원문의 서로 다른 측면(인증 여부 vs 장치 역할 분류)을 가리킨다. 정보 자체가 다른 Key에 이미 존재한다는 이유로 이 Key를 임의로 비워두지 않고, LA7.16i 자신의 OM 원문에 명시된 값이므로 그대로 기재했다(값 중복이지 재추론이나 타 제품 값 차용이 아님).
...
**섹션 전체 비적용**: LA7.16i는 Dante가 아닌 Milan-AVB(L-Acoustics 고유 인증)/AES67 오디오 네트워크를 사용한다. 이 섹션은 5D(d&b audiotechnik Dante 설치용 라인) 파싱 중 신설되었으며, Milan-AVB 기준으로 설계된 input_avb 섹션과 별도로 Dante 프로토콜 전용 섹션으로 분리되었다. LA7.16i의 기존 소스에는 이 개념에 대응하는 서술이 없어 전량 null로 소급 반영한다.
...
| Key | Value | Unit |
|---|---|---|
| Network_IP_Default [1] | 192.168.1.100 / 255.255.255.0 | - |

### 주석 및 출처 (Notes & Sources)

**출처**: OM p.32 "Installation > L-NET/AVB or AES67" (v2.4에서 확보) / AE에는 대응 서술 없음.

**[1][구조 개편 v2.0 신설 근거]**: v1.x까지 "d90_specific" 섹션에 있던 Key. IP 기본값/네트워크 설정은 AVB/Milan 오디오 스트림(input_avb)이나 물리적 커넥터(connectivity)와는 성격이 다른 별도 주제이므로, 이 재배치를 계기로 "network"라는 주제 기반 신규 섹션을 신설했다(D80_v2.0.md/D90_v2.0.md에도 동일 섹션이 반영됨).

**[v2.4 재검증] Network_IP_Default 실값 확보**: OM p.32 "Installation > L-NET/AVB or AES67" 절이 "The factory default IP settings of all L-Acoustics devices are: IP address: 192.168.1.100 / Subnet address: 192.168.1.0/24 / IP broadcast address: 192.168.1.255 / Subnet mask: 255.255.255.0"이라고 명시한다. LA7.16i도 이 L-NET 제어망을 사용하는 L-Acoustics 장비이므로 이 값이 LA7.16i 자신에게 적용됨을 OM 본문에서 직접 확인했다(브랜드 공통 문구이나 LA7.16i 자신의 OM에 실린 서술이므로 원본성 요건 충족). D90의 표기 형식(IP/서브넷마스크 병기, "192.168.1.90 / 255.255.255.0")에 맞춰 "192.168.1.100 / 255.255.255.0"으로 기재했다. AE는 IP 기본값을 언급하지 않는다(OM 단독 출처).
...
**[10a][v2.4 재검증] Output_Mode_Selectable**: OM 전량을 "output mode"/"bridge"/"channel pair"/"grouping"/"mono"/"stereo" 키워드로 재검색했다. LA7.16i의 8개 스피커 터미널 블록은 이미 Bridging_Support="No"(각주[1], OM p.16/p.34 명시적 확정)에 따라 전량 SE(단일 채널) 고정 배선이며(Speaker_Terminal_Block_SE_BTL_Wiring 참조), 이 SE 고정 사실 외에 D90의 "Dual Channel/Mix TOP-SUB/2-Way Active" 같은 사용자 선택형 출력 채널 모드(그룹핑/믹스 모드) 자체를 서술하는 문장은 OM/AE에 없다. 다만 이는 브릿지 미지원처럼 명시적으로 부인된 것이 아니라 단순히 서술이 없는 것이므로, 확정 비존재(No)가 아닌 미확인(null)으로 보수적으로 유지한다.

**[11][구조 개편] Fault_Contact_Type**: 5D(d&b audiotechnik) 파싱 중 신설된 Key(FAULT 릴레이 접점, NO/C/NC). LA7.16i의 기존 소스에는 이 개념에 대응하는 서술이 없어 null로 소급 반영한다.
...
**[13a][v2.4 재검증] Signal_Path_Management**: OM 전량을 "signal path"/"fallback"/"override"/"priority"/"emergency"/"announcement"/"AutoStandby"/"AutoWakeup" 키워드로 재검색했다. OM p.14 "Technical description > Signal processing and amplification > Signal inputs" 절에 "The AUX input source can be used as main input source, secondary input source, or fallback input source"라는 fallback 개념이 확인되나, 이는 이미 input_avb 섹션의 Fallback_Modes Key("AVB or AES67 to AES/EBU or analog (shared input), user-defined mapping")로 포착된 것과 동일한 서술이다. d&b 유래 Signal_Path_Management 개념(Override 긴급방송 우선 경로, AutoStandby/AutoWakeup 자동 절전/기상)에 대응하는 서술은 LA7.16i의 OM/AE 어디에도 없다. 동일 개념을 Fallback_Modes와 Signal_Path_Management 두 Key에 중복 기재하지 않고, 신규 정보가 없는 이 Key는 미확인(null)으로 유지한다.

**[32] Web_Remote_Interface**: 10D(d&b audiotechnik 차세대 설치용 라인) 파싱 중 발견된 신규 Key — 별도 소프트웨어(LA Network Manager) 없이 표준 웹 브라우저로 기기에 직접 접속하는 브라우저 기반 내장 웹 인터페이스 개념. LA7.16i의 기존 소스에는 이 개념에 대응하는 서술이 확인되지 않아 null(미확인)로 소급 반영한다.
...
**[14a][v2.4 재검증] Storage_Temp_Range/Storage_Humidity**: OM p.6(안전 지침) 및 p.63(Specifications > Operating conditions) 절을 "storage" 키워드로 재검색했으나, Operating_Temp_Range(-5~50℃)/Max_Operating_Altitude만 확인되며 비가동(보관) 상태의 온도/습도 범위는 OM/AE 어디에도 별도로 명시되어 있지 않다. 미확인(null)으로 유지한다.
...
이번 파일에서 null 처리된 항목 (총 43건, D25 파싱 중 발견된 신규 Key Max_Output_Current_Peak 1건, 5D 파싱 중 발견된 신규 Key 8건(input_dante 7개 + Fault_Contact_Type) 및 10D 파싱 중 발견된 신규 Key 1건(Web_Remote_Interface) 소급 반영 포함):

**비적용(제품 아키텍처상 해당 없음, 총 12건)**: Rated_Power_Per_Ch_BTL, Peak_Power_Per_Ch_BTL, Rated_Power_Per_Ch_PBTL, Peak_Power_Per_Ch_PBTL, PBTL_Activation_Method — 브릿지 모드 확정적 미지원(Bridging_Support="No", OM p.16/p.34 명시적 경고 2건 + OM/AE 전량 검색 근거, 각주[1])에 따라 해당 동작 모드의 측정값 자체가 정의되지 않아 null 처리(v1.0 대비 10종에서 4종으로 통합, 각주[1a] 참조). Dante_RX_Channels, Dante_Sampling_Rates, Dante_SRC, Dante_Redundancy, Dante_Network_Flows, Dante_Latency, Dante_IP_Architecture(5D 파싱 중 신설된 input_dante 섹션 7개 Key 전부 — LA7.16i는 Dante가 아닌 Milan-AVB/AES67을 사용하므로 섹션 전체 비적용, input_dante 섹션 주석 참조).

**미확인(LA7.16i 자체 소스 근거 부족, 총 15건)**: Rated_Power_Per_Ch_2.7_Ohms, Total_Power_Capability, Max_Output_Current_Peak(D25 파싱 중 발견된 신규 Key 소급 반영), THD_N_4ohm, EQ_Filters_Per_Output, Preset_Memory_User, Preset_Memory_Factory, XLR_Pin_Polarity, SpeakON_Left_Pinout, SpeakON_Right_Pinout, CACOM_Pinout, SC32_Channel_Mapping, Speaker_Output_Accessories, Fault_Contact_Type(5D 파싱 중 신설된 connectivity 섹션 신규 Key 소급 반영, 각주[11] 참조), Web_Remote_Interface(10D 파싱 중 신설된 control_monitoring 섹션 신규 Key 소급 반영, 각주[32] 참조).

**미확인 — v2.4에서 LA7.16i 자체 소스로 재검증 완료(총 16건)**: input_analog(CMRR_Analog_Input), input_digital(Digital_Input_Impedance), audio_performance(SN_Ratio_Analog_Input, SN_Ratio_Digital_Input, Output_Noise_Dynamic_Range_Detail), dsp(System_Start_Up_Time, LoadMatch_Max_Cable_Length, Load_Monitoring_System_Check, Time_To_Tone, Energy_Saving_Detail, AmpPresets_Detail), operating_conditions(Storage_Temp_Range, Storage_Humidity), control_monitoring(Signal_Path_Management), connectivity(Output_Mode_Selectable), power_supply(Mains_Current_Limiter_Range) — 이 16개 Key는 v2.0 구조 개편(d&b d80_specific/d90_specific 섹션으로부터 재배치) 당시 LA7.16i의 OM/AE를 재조회하지 않고 위치만 이동했던 것이다. v2.4에서 LA7.16i의 OM(v5.0, 69p 전량) 및 AE(docx 전량)를 각 Key별 키워드로 실제 재검색했으나 어느 Key에 대해서도 대응 서술을 발견하지 못했다(각 섹션 각주 참조). 이는 "출처를 재확인하지 않아 미확인"이 아니라 "출처를 재확인했으나 서술이 없어 미확인"으로, 근거 성격이 바뀌었을 뿐 확정된 비존재(0/No)로 격상할 근거(원문의 명시적 부인)는 없으므로 미확인(null) 분류는 유지한다. 같은 18개 Key 중 나머지 2건(input_avb의 AVB_Device_Type, network의 Network_IP_Default)은 재검증 결과 LA7.16i 자신의 OM에서 실값을 확보하여 각 섹션에 값이 채워졌으므로 이 Null Report에서 제외되었다(각주 참조).
...
v2.0 to v2.1: (Minor, null-only 스키마 동기화) 5D_v1.0.md(d&b audiotechnik Dante 설치용 라인) 파싱 중 신규 발견된 두 스키마 확장을 양방향 동기화 규칙에 따라 LA7.16i에 소급 반영했다. (1) **input_dante 신규 섹션 추가**(input_avb 섹션과 network 섹션 사이): Dante_RX_Channels, Dante_Sampling_Rates, Dante_SRC, Dante_Redundancy, Dante_Network_Flows, Dante_Latency, Dante_IP_Architecture 7개 Key 전부 null — LA7.16i는 Dante가 아닌 Milan-AVB(L-Acoustics 고유 인증)/AES67을 사용하므로 섹션 전체가 비적용이다. (2) **connectivity 섹션에 Fault_Contact_Type Key 추가**(PBTL_Activation_Method 다음 마지막 행, 각주[11]): null — LA7.16i의 기존 OM/AE 소스에는 FAULT 릴레이 접점(NO/C/NC) 개념에 대응하는 서술이 없다. 이번 개편은 LA7.16i 자신의 실제 데이터(값)에는 어떠한 영향도 주지 않는 순수 null-Key 추가이며, 섹션 계층 구조상 신규 섹션(input_dante)이 하나 늘었으나 기존 섹션들의 내부 구조나 값은 전혀 바뀌지 않아 Minor 버전으로 분류했다(SKILL v1.8 "신규 브랜드 투입으로 [브랜드]_specific 섹션이 통째로 추가되는 경우도... Minor로 분류" 원칙과 동일 논리 적용). Null Report 총 건수는 36건에서 44건으로 변경(input_dante 7개 Key는 "비적용"으로, Fault_Contact_Type 1개 Key는 "미확인"으로 분류 — 순증가 8건).

v2.1 to v2.2: (Minor, null-only 스키마 동기화) 10D_v1.0.md(d&b audiotechnik 차세대 설치용 라인) 파싱 중 신규 발견된 Key `Web_Remote_Interface`를 control_monitoring 섹션의 Signal_Path_Management[13a] 다음 마지막 행에 null로 소급 반영했다(각주[32] — 이 파일에서 이미 사용 중인 [14](Fan_Count)와 충돌하지 않도록 신규 번호 부여) — 별도 소프트웨어(LA Network Manager) 없이 표준 웹 브라우저로 기기에 직접 접속하는 브라우저 기반 내장 웹 인터페이스 개념이며, LA7.16i의 기존 OM/AE 소스에는 대응 서술이 없어 미확인(null)으로 처리했다. 이번 개편은 LA7.16i 자신의 실제 데이터(값)에는 어떠한 영향도 주지 않는 순수 null-Key 1건 추가이므로 Minor 버전으로 상향했다. Null Report 총 건수는 44건에서 45건으로 변경(신규 1건, 미확인).

v2.2 to v2.3: (표기 정리, Minor) 프로젝트 전체 재검토에서 [신규] 각주 태그가 추가 시점 이후 버전이 계속 올라가면서 더 이상 "신규"가 아닌데도 방치되어 있던 문제가 발견되어, 각주 번호(예: [32])는 유지한 채 [신규] 텍스트만 전면 제거했다. 데이터/Key/Value/Unit/Null Report는 전혀 변경되지 않았다.

v2.3 to v2.4: (판단 근거의 원본성 요건 이행, Minor) v2.0 구조 개편(d&b d80_specific/d90_specific 그랩백 섹션 폐지) 당시 위치만 이동되고 "LA7.16i의 OM/AE를 재조회하지 않았다"는 이유로 미확인(null) 처리되었던 18개 Key 전부를 이번 세션에서 LA7.16i 자신의 원본 소스(OM v5.0 PDF 69p 전량을 pdftotext -layout으로 텍스트 추출, AE docx 전량을 word/document.xml에서 텍스트 추출)로 실제 재검색했다(SKILL v1.11 "판단 근거의 원본성 요건" 이행). 결과: (1) **실값 확보 2건** — network 섹션 Network_IP_Default: OM p.32 "Installation > L-NET/AVB or AES67" 절의 "The factory default IP settings of all L-Acoustics devices are: IP address: 192.168.1.100... Subnet mask: 255.255.255.0"를 근거로 "192.168.1.100 / 255.255.255.0"으로 기재(각주[1] 갱신). input_avb 섹션 AVB_Device_Type: OM p.64 "Specifications > Milan-AVB" 스펙 표의 "Featured AVB entities: MILAN™-compliant, Avnu™-compliant Milan-AVB Bridge and Listener" 문구를 근거로 "Bridge and Listener"로 기재(각주[8a] 갱신) — 이 값은 같은 섹션 AVB_Certification Key에 이미 반영된 것과 동일 출처이나, D90처럼 두 Key가 서로 다른 값을 가질 수 있는 별도 Key 구조이므로 LA7.16i 자신의 원문에 근거해 그대로 기재했다(값 중복이지 재추론이나 타 제품 값 차용이 아님). (2) **재검증 후 미확인 유지 16건** — input_analog(CMRR_Analog_Input), input_digital(Digital_Input_Impedance), audio_performance(SN_Ratio_Analog_Input, SN_Ratio_Digital_Input, Output_Noise_Dynamic_Range_Detail), dsp(System_Start_Up_Time, LoadMatch_Max_Cable_Length, Load_Monitoring_System_Check, Time_To_Tone, Energy_Saving_Detail, AmpPresets_Detail), operating_conditions(Storage_Temp_Range, Storage_Humidity), control_monitoring(Signal_Path_Management), connectivity(Output_Mode_Selectable), power_supply(Mains_Current_Limiter_Range) — OM/AE 전량을 각 Key별 관련 키워드(CMRR/common-mode, AES/EBU impedance, S/N·SNR, start-up·boot, LoadMatch·load monitoring, time to tone, energy saving·eco mode·standby, amp preset, storage temperature·humidity, signal path·fallback·override·redundancy, output mode·bridge, mains current limiter·inrush 등)로 재검색했으나 어느 항목도 대응 서술을 발견하지 못했다(각 섹션 각주에 재검색 근거 및 인접 개념과의 구분 사유를 상세 기재). 기존 각주의 "재조회하지 않고 위치만 옮겼다"는 문구를 전량 삭제하고 "재조회했으나 서술 없음"으로 갱신했다. 원문이 개념을 명시적으로 부인한 것이 아니라 단순히 언급이 없는 것이므로, 확정 비존재(0/No)로 격상하지 않고 미확인(null) 분류를 유지했다(SKILL 원칙: 확정 비존재는 원문의 명시적 부인이 있을 때만 사용). 이번 작업으로 다른 ~15+18개 기존 해결 Key(Bridging_Support, AES67_Support 등)는 전혀 건드리지 않았다. Null Report 총 건수는 45건에서 43건으로 변경(실값 확보 2건만큼 감소).
