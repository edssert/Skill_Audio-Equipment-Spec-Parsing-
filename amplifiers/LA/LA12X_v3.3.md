...
Schema_Version: 3.3 (재조사 - Bridging_Support/AES67_Support를 LA12X 자신의 SPS 원본 재조회로 확정(No), 구조 변경 없는 Minor 버전)
최종 작업 일시: 2026-07-18
...
**[18] Bridging_Support=No(재조사 완료, v3.3, 2026-07-18)**: v2.9~v3.2까지는 "원본 문서를 재조회하지 않고 기존 파싱 결과 텍스트만으로 판단"했다는 사유로 미확인(null) 유지했으나, 이번에 LA12X 자신의 SPS(LA12X_SPS_EN_2.5.pdf) p.3 "AMPLIFIED CONTROLLERS – THE RANGE" 비교표를 직접 재조회(페이지 이미지 렌더링으로 육안 확인)해 확정했다. 표 각주 "* Any odd/even pair of outputs can be bridged (BTL)"의 별표(*)는 LA1.16i 열의 출력 전력값("8* x 300 W")에만 붙어 있고, LA12X 열(4x1400W/4x2600W/4x3300W)에는 별표가 전혀 없다 — 즉 BTL 브릿징은 LA1.16i 전용 기능이며 LA12X는 이 비교표 자체에서 명시적으로 제외되어 있어 확정적 비존재(No)로 처리한다. Rated/Peak_Power_Per_Ch_BTL/PBTL 4개 Key는 브릿지 자체가 없으므로 계속 null 유지(구조적 비적용에 가까우나, PBTL은 LA12X 자신의 문서에서 별도로 전혀 언급되지 않아 완전히 다른 개념일 가능성도 있어 보수적으로 null 유지, 비적용으로 재분류하지 않음).
...
**PSU_Mains_Rated_Power**: LA7.16 v1.1 재검토 과정에서 신설된 Key(SMPS 연속 메인즈 입력 전력 정격, LA7.16은 OM에 "2800 W"로 명시. D80은 "7000 W, short term RMS" 조건으로 명시, D80_v1.0.md power_supply 섹션 참조). LA12X의 기존 소스(OM v10.1, SPS 2.5, AE)에는 이에 대응하는 명시적 "Mains rating ... W" 단일 수치가 확인되지 않아 null로 처리한다. 참고로 LA12X의 `Total_Power_Capability`(12000 W)는 이 PSU_Mains_Rated_Power와는 다른 개념(총 출력 용량)으로 판단되며 혼동하여 대입하지 않았다.
...
**[14] AES67_Support=No (재조사 완료, v3.3, 2026-07-18)**: L-Acoustics "i" 설치전용 라인(LA1.16i, LA7.16i)에서 발견된 신규 속성 — Milan-AVB와 AES67을 사용자가 선택 가능한 이중 모드 제품군에서 도입된 Key. v2.9~v3.2까지는 "원본을 재조회하지 않고" null(미확인) 유지했으나, 이번에 LA12X 자신의 SPS(LA12X_SPS_EN_2.5.pdf) p.3 "AMPLIFIED CONTROLLERS – THE RANGE" 비교표를 직접 재조회(페이지 이미지 렌더링으로 육안 확인)해 확정했다. 표의 "Input channels" 행에서 LA1.16i/LA2Xi/LA7.16(i) 열은 "4 x AVB or AES67" 형태로 AES67을 명시하는 반면, LA12X 열은 "4 x AVB / 4 x Analog / 4 x AES/EBU"로만 기재되어 있고 AES67 언급이 전혀 없다 — L-Acoustics 자신의 공식 비교표가 LA12X를 AES67 미지원으로 명시적으로 구분하고 있으므로 확정적 비존재(No)로 처리한다.
...
| Service_Port_Type [15] | null | - |
| Speaker_Terminal_Block_SE_BTL_Wiring [14] | null | - |
| PBTL_Activation_Method [14] | null | - |
| Fault_Contact_Type [17] | null | - |
...
**SC32_Channel_Mapping, Speaker_Output_Accessories**: LA7.16 파싱 중 발견된 신규 속성. LA12X는 speakON/CA-COM 개별 커넥터 구조를 사용하며 SC32 통합 커넥터를 사용하지 않으므로 해당 없음(null) 처리한다.

**Terminal_Block_Pinout**: LA7.16 v1.3에서 12-point terminal block(AES/ANA IN+LINK, GPIO, 24V DC backup)의 상세 핀맵이 추가되며 신설된 Key. LA12X는 이런 통합 단자대 구조를 사용하지 않고 전용 XLR3 커넥터(Input_Connectors, XLR_Pin_Polarity 참조)를 사용하므로 해당 없음(null) 처리한다.

**[15] Service_Port_Type (v2.9, Phase 4 동기화)**: "i" 설치전용 라인에서 발견된 IP 설정용 USB 서비스 포트(USB-C 또는 USB Micro-B) Key. LA12X의 기존 소스에는 USB 서비스 포트 관련 서술이 확인되지 않아 null 처리한다.
...
**[17] Fault_Contact_Type**: 5D(d&b audiotechnik) 파싱 중 신설된 Key(FAULT 릴레이 접점, NO/C/NC). LA12X의 기존 소스에는 이 개념에 대응하는 서술이 없어 null로 소급 반영한다.
...
**Web_Remote_Interface (10D 파싱 중 신설, 양방향 동기화로 null 반영)**: 10D(d&b audiotechnik 차세대 설치용 라인) 파싱 중 발견된 신규 Key — 별도 소프트웨어(R1/ArrayCalc, L-Acoustics의 경우 LA Network Manager) 없이 표준 웹 브라우저로 앰프에 직접 접속하는 브라우저 기반 내장 웹 인터페이스 개념. LA12X의 기존 소스에는 이 개념에 대응하는 서술이 확인되지 않아 null(미확인)로 소급 반영한다.
...
**Max_Operating_Altitude**: LA7.16 파싱 중 발견된 신규 속성(OM 스펙 표에 "Maximum altitude 2000 m"로 명시). LA12X의 기존 소스에는 최대 고도 스펙이 확인되지 않아 null로 처리한다.
...
이번 소스 집합에서 정의되지 않아 null 처리된 항목: Rated_Power_Per_Ch_16_Ohms, Peak_Power_Per_Ch_4_Ohms, Peak_Power_Per_Ch_8_Ohms, Peak_Power_Per_Ch_16_Ohms, Per_Channel_DSP_Tools, Amplifier_Power_Management, SC32_Channel_Mapping, Speaker_Output_Accessories, GPIO_Count, GPIO_Type, DSP_Backup_Power_Input, Max_Operating_Altitude, PSU_Mains_Rated_Power, Terminal_Block_Pinout (Phase 3-1(LA7.16), 총 14건), CMRR_Analog_Input, Digital_Input_Impedance, SN_Ratio_Analog_Input, SN_Ratio_Digital_Input, System_Start_Up_Time, Storage_Temp_Range, Storage_Humidity, LoadMatch_Max_Cable_Length, Signal_Path_Management, Load_Monitoring_System_Check, Output_Mode_Selectable, Mains_Current_Limiter_Range (미확인 — 원 Phase 3-2(D80) 신설분, 구조 개편 v3.0으로 각 해당 섹션에 재배치되었으나 Key/Value/Unit은 불변, 총 12건), AVB_Device_Type, Network_IP_Default, Time_To_Tone, Energy_Saving_Detail, AmpPresets_Detail, Output_Noise_Dynamic_Range_Detail (미확인 — 원 Phase 3-1(D90) 신설분, 구조 개편 v3.0으로 각 해당 섹션에 재배치 — Milan_Device_Type은 AVB_Device_Type으로 개명, Milan_Input_Channel_Streams/Milan_Redundancy/Milan_Routable_Inputs 3건은 기존 AVB_Channels/AVB_Stream_Count/Redundancy_Type Key에 흡수되어 별도 항목에서 제외, 총 6건), Rated_Power_Per_Ch_BTL, Peak_Power_Per_Ch_BTL, Rated_Power_Per_Ch_PBTL, Peak_Power_Per_Ch_PBTL, Service_Port_Type, Speaker_Terminal_Block_SE_BTL_Wiring, PBTL_Activation_Method (Phase 4(Upload 4개 제품) — BTL/PBTL 관련 Key는 구조 개편 v3.0으로 10건에서 4건으로 통합, 총 7건 — Bridging_Support와 AES67_Support는 v3.3에서 재조사 완료되어 이 목록에서 제외됨), Max_Output_Current_Peak (D25 파싱 중 발견된 신규 Key, amplification 섹션에 소급 반영, 1건), Dante_RX_Channels, Dante_Sampling_Rates, Dante_SRC, Dante_Redundancy, Dante_Network_Flows, Dante_Latency, Dante_IP_Architecture (비적용 — 5D 파싱 중 신설된 input_dante 섹션 전체, LA12X는 Milan-AVB를 사용하고 Dante를 사용하지 않아 섹션 전체 비적용, 총 7건), Fault_Contact_Type (미확인 — 5D 파싱 중 신설된 connectivity 섹션 신규 Key, LA12X 기존 소스에 대응 서술 없음, 1건), Web_Remote_Interface (미확인 — 10D 파싱 중 신설된 control_monitoring 섹션 신규 Key, LA12X 기존 소스에 대응 서술 없음, 1건). 전체 49건(v3.2 시점 51건에서 Bridging_Support/AES67_Support 재조사 완료로 2건 감소).
...
v3.1 to v3.2: (양방향 동기화, Minor) 신규 제품 10D(d&b audiotechnik, 차세대 설치용 라인) 파싱 중 발견된 신규 Key `Web_Remote_Interface`를 control_monitoring 섹션에 null로 소급 반영했다 — Signal_Path_Management 다음 마지막 행으로 추가했다. 별도 소프트웨어(R1/ArrayCalc, L-Acoustics의 경우 LA Network Manager) 없이 표준 웹 브라우저로 앰프에 직접 접속하는 브라우저 기반 내장 웹 인터페이스 개념으로, LA12X의 기존 소스(OM v10.1, SPS 2.5, AE)에는 이에 대응하는 서술이 확인되지 않아 null(미확인)로 처리했다. 순수 스키마 동기화이며 LA12X의 기존 Key/Value/각주/섹션 구조는 일절 변경하지 않았다 — 구조 재편이 아닌 신규 null Key 1건 추가에 불과하므로 Minor 버전(v3.2)으로 처리했다. Null Report 총 건수는 50건에서 1건 증가하여 51건이 되었다(Web_Remote_Interface 1건 미확인).

v3.2 to v3.3: (재조사, Minor) 프로젝트 전체 "핑계성 미확인" 전수조사에서 v2.9 각주가 "원본 문서를 재조회하지 않고 기존 파싱 결과 텍스트 기반으로 판단"했다고 스스로 명시한 상태로 남아있던 Bridging_Support/AES67_Support 2개 Key를 LA12X 자신의 SPS(LA12X_SPS_EN_2.5.pdf) p.3 "AMPLIFIED CONTROLLERS – THE RANGE" 비교표 재조회(페이지 이미지 렌더링)로 재확인했다. 두 Key 모두 L-Acoustics 자신의 공식 비교표가 LA12X를 명시적으로 구분(BTL 각주 별표는 LA1.16i 열에만, AES67은 LA1.16i/LA2Xi/LA7.16(i) 열에만 표기)하고 있어 확정적 비존재(No)로 값을 채웠다(각주 [18], [14] 갱신). BTL/PBTL 전력 Key 4개는 브릿지 자체가 없어 계속 null 유지. Value 갱신 2건 외 기존 Key/섹션 구조는 변경하지 않았으므로 Minor 버전(v3.3)으로 처리했다. Null Report 총 건수는 51건에서 2건 감소하여 49건이 되었다. 같은 버전에서 프로젝트 전체 재검토 결과 발견된 `[신규]` 각주 태그(추가 시점 이후 버전이 계속 올라가며 더 이상 "신규"가 아닌데도 방치된 표기 문제)도 함께 정리 — 각주 번호(예: [14], [17])는 유지한 채 `[신규]` 텍스트만 전면 제거했다. 데이터/Key/Value/Unit/Null Report는 이 정리로 인해 전혀 변경되지 않았다.
