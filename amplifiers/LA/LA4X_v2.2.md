...
Schema_Version: 2.2 (5D 파싱 중 신설된 input_dante 섹션 및 Fault_Contact_Type Key + 10D 파싱 중 신설된 control_monitoring Web_Remote_Interface Key의 양방향 스키마 동기화 반영 — null-only 추가, 실제 LA4X 데이터 변경 없음)
최종 작업 일시: 2026-07-16
...
| Web_Remote_Interface [신규][27] | null | - |
...
**[신규][27] Web_Remote_Interface**: 10D(d&b audiotechnik 차세대 설치용 라인) 파싱 중 발견된 신규 Key — 별도 소프트웨어(LA Network Manager) 없이 표준 웹 브라우저로 기기에 직접 접속하는 브라우저 기반 내장 웹 인터페이스 개념. LA4X의 기존 소스에는 이 개념에 대응하는 서술이 확인되지 않아 null(미확인)로 소급 반영한다.
