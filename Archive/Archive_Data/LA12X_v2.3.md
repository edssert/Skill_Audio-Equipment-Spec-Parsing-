...
Schema_Version: 2.3 (재분류됨, 구 v5.0 - Phase 3-1 - schema synchronized with LA7.16: 12 new keys backfilled as null; no existing values changed)

버전 재분류 안내 (SKILL v1.8 시맨틱 버저닝 소급 적용): 이 파일은 원래 v5.0으로 발행되었으나, LA7.16 파싱에서 발견된 신규 Key 12개를 null로 추가한 것은 하위 호환 변경(스키마 확장)이므로 Minor 버전(v2.3)으로 재분류되었다. 이하 "버전 변경 이력" 절의 서술은 발행 당시 원문 그대로 보존한다.
...
본 파일은 Phase 3-1 (동종 브랜드 이기종 채널) 테스트에서 LA7.16(16채널) 파싱 중 발견된 신규 속성(Key) 12개를 `null` 값으로 소급 반영한 결과물이다. 기존 v4.0의 값은 일절 변경하지 않았으며, 신규 Key만 각 섹션에 [신규] 표시로 추가했다. 상세 근거는 LA7.16_v1.0.md의 각 섹션 각주를 참조.
...
v3.0 to v4.0: (누락 수정) 저항 표기 표준화 규칙이 Value에만 적용되고 Key에는 미적용되었던 오류를 정정. `Rated_Power_Per_Ch_2R7` -> `Rated_Power_Per_Ch_2.7_Ohms`, `Rated_Power_Per_Ch_4R` -> `Rated_Power_Per_Ch_4_Ohms`, `Rated_Power_Per_Ch_8R` -> `Rated_Power_Per_Ch_8_Ohms`로 Key 명명 전면 수정. 이후 모든 Phase에서 저항 조건이 포함된 Key는 예외 없이 이 표준을 따른다.

v4.0 to v5.0: (Phase 3-1 동기화) LA7.16(16채널) 파싱 과정에서 발견된 신규 속성 12개(Rated_Power_Per_Ch_16_Ohms, Peak_Power_Per_Ch_4/8/16_Ohms, Per_Channel_DSP_Tools, Amplifier_Power_Management, SC32_Channel_Mapping, Speaker_Output_Accessories, GPIO_Count, GPIO_Type, DSP_Backup_Power_Input, Max_Operating_Altitude)를 각 해당 섹션에 `null` 값으로 추가. 기존 v4.0의 값과 각주는 일절 변경하지 않았다(무결성 유지).
