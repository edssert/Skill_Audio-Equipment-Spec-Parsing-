...
Schema_Version: 2.2 (재분류됨, 구 v4.0 - Phase 2 - key naming fix per skill update v0.4: resistance notation now applied to both Key and Value)

버전 재분류 안내 (SKILL v1.8 시맨틱 버저닝 소급 적용): 이 파일은 원래 v4.0으로 발행되었으나, 저항 표기 Key 명명 규칙의 전면 재적용은 다수 Key에 영향을 주지만 스키마 계층 구조 자체는 바뀌지 않았으므로 Minor 버전(v2.2)으로 재분류되었다. 이하 "버전 변경 이력" 절의 서술은 발행 당시 원문 그대로 보존한다.
