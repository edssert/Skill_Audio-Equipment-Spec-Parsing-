...
**Skill_Version: 1.10** (파일명 관례: SKILL_v1.10.md)
...
- **Phase 4**: 스키마 진화 및 하위 호환성 — Phase 3의 동기화 원칙을 표준 운영 절차로 고정한다. 누적 제품 수가 많아지면(대략 5개 이상) 매번 전량 재출력하는 대신 "변경된 Key와 영향받는 파일 목록"을 먼저 요약하고, 사용자 확인 후 일괄 출력할지 개별 확인 후 출력할지 묻는다. 현재 누적 제품 수는 4개(LA12X, LA7.16, D80, D90)로 아직 Phase 4 전환 기준(5개)에 도달하지 않았다.
</phase_workflow>
