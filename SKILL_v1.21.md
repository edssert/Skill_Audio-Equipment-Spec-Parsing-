...
**Skill_Version: 1.21** (파일명 관례: SKILL_v1.21.md)
...
4. **무결성 유지 (엄격)**: 없는 스펙은 억지로 만들지 않고 `null` 처리한다. 소스 간 값이 다르면 어느 한쪽도 버리지 않고 둘 다 보존한다(자세한 규칙은 "데이터 충돌 보존" 절 참조).
   ...
   - **확정된 비존재(0)와 미확인(null)의 구분 (v1.9 신규)**: 소스 문서 전체를 스캔했음에도 특정 기능/커넥터에 대한 언급이 전혀 없어 "그 기능 자체가 제품에 존재하지 않는다"고 합리적으로 판단할 수 있는 경우(예: GPIO 관련 용어가 매뉴얼 전체에서 0건 검출), 이는 단순 `null`(정보 부족으로 미확인)이 아니라 개수형 Key라면 `0`(확정적 비존재)으로 명시할 수 있다. 다만 이 판단은 해당 소스 문서를 전량 스캔(전문 검색 등)한 근거가 있을 때만 내리며, 근거를 각주에 명기해야 한다. 단순히 일부 챕터만 훑고 못 찾았다고 0으로 단정하지 않는다.
     ...
     - **각주 서술 전반에 대한 원본성 요건 확장 (v1.15 신규)**: 위 원본성 요건은 0/null 판단에만 국한되지 않고, "주석 및 출처" 각주에 적는 모든 설명적 서술에 동일하게 적용된다. 각주는 검증되지 않은 일반 도메인 지식이나 통칭적 카테고리 주장("self-powered 스피커는 항상 ~하다", "이 업계에서는 보통 ~다" 등)을 사실인 것처럼 단정적으로 적지 않는다 — 그런 서술이 실제로 필요하다면 반드시 해당 제품 자신의 원본 문서에서 확인된 근거(전량 스캔 결과, 특정 페이지의 명시적 서술 등)로 대체해서 적는다. 예: "self-powered 스피커에는 스피커 레벨 커넥터가 없다"처럼 제품군 전체에 대한 일반화 주장을 적지 않고, "이 제품 자신의 OM/AE/SPS를 'PA-COM'/'NLT4' 키워드로 전량 스캔한 결과 0건 검출되었고, 실제 커넥터는 X/Y Key로 이미 표현되어 있다"처럼 그 제품 자신의 문서 확인 결과로 서술한다(PANTHER의 Input_Connector/PA_COM_Pinout_* 삭제 근거를 "self-powered 스피커는 항상 라인 레벨만 받는다"는 일반화 주장으로 잘못 서술했다가, 실제로는 PANTHER 자신도 디지털(Ethernet) 입력이 있어 그 주장 자체가 틀렸음이 드러난 사례에서 확립 — 사용자가 "이런 문장은 어떤 메뉴얼에도 없잖아"라고 지적).
     - **"시간 제약/시간 배분상" 사유 전면 금지 (v1.17 신규, 엄격)**: null 처리의 사유로 "시간 제약상", "시간 배분상", "남은 제품 전체 조사 일정상" 등 조사 주체(에이전트)의 작업 속도·일정을 이유로 든 표현을 어떤 각주에도 쓰지 않는다. 이런 표현은 "그 정보가 원본에 없다"는 사실이 아니라 "찾아보지 않았다"는 작업 태만을 자백하는 것이며, Null Report의 신호(원본에 정보가 정말 없는지 vs 단순히 안 찾아본 것인지)를 오염시킨다. 제품 하나를 파싱하기 시작했다면 그 제품에 배정된 모든 섹션(preset_guide_and_matching, delay_defaults, mechanical_safety 포함)을 실제로 원본 문서에서 끝까지 조사한 뒤에 결과를 적는다 — 여러 제품을 동시에 빠르게 처리해야 하는 상황이라도 각 제품 내부에서 섹션을 건너뛰지 않는다. 조사 결과 원본에 정말 없거나(구조적으로 이 제품이 해당 개념의 1차 엘리먼트가 될 수 없는 경우 등) 원본을 다 뒤졌는데도 못 찾은 경우에만 null로 남기고, 그 사유는 반드시 "원본의 어느 챕터/페이지를 확인했는지"와 "왜 없다고 판단했는지"로 구체적으로 서술한다(예: "OM p.49-52 'Loudspeaker configurations' 챕터 전체를 확인했으나 이 조합에 대한 pre-alignment delay 표가 없고 'geometric delays만 추가하라'는 서술만 있음" — 이런 서술은 허용되는 미확인이다). 이 규칙이 도입된 계기: A15/A10i/A15i Focus·Wide 6개 파일과 KS21/SB6i 등에서 delay_defaults·mechanical_safety를 "시간 배분상 미확인"으로 남겼다가, 실제로 원본을 조사하자 대부분 명확한 실값이 존재했음이 드러난 사례(사용자 지적 2026-07-17, "이정도면 한번 한제품 파싱할때 똑바로해").
...
- **동기화 대상 Key의 범위 제한 — "브랜드 고유 물리/명칭 Key"는 동기화하지 않는다 (v1.14 신규)**: 위의 "특정 부하 조건" 같은 예시는 부하 임피던스처럼 *브랜드 무관하게 어느 제조사든 보고할 수 있는 범용 측정/스펙 개념*에 한정된다. 이와 구분되는 별도 유형이 있다 — Key 이름 자체가 **한 브랜드/제품의 고유한 물리적 설계나 부품·커넥터 모델명을 그대로 지칭**하는 경우다(예: d&b GSL의 전방/측면 LF 드라이버 배선 채널 구분을 그대로 옮긴 `Front_LF_Transducer`/`Side_LF_MF_HF_Nominal_Impedance`, GSL의 커넥터 모델명을 그대로 딴 `NLT4_Pinout_Pin1`, L-Acoustics의 커넥터 모델명을 그대로 딴 `PA_COM_Pinout_AB`, d&b의 명명된 기능을 그대로 딴 `CUT_Mode_*`). 이런 Key는:
  ...
  - **아키텍처상 카테고리 자체가 성립하지 않는 Key도 동일하게 삭제 대상 (v1.15 보강)**: 브랜드 고유 명칭이 아니어도, Key가 가리키는 개념의 카테고리 자체가 특정 제품 유형에서 성립할 수 없는 경우(예: 스피커 레벨 다채널 분배 커넥터인 PA-COM/NLT4류는 애초에 외부 앰프로부터 이미 증폭된 신호를 받는 패시브/하이브리드 토폴로지에서만 쓰이는 커넥터 카테고리)도 동일하게 null로 남기지 않고 삭제한다. 다만 이런 판단의 근거는 반드시 그 제품 자신의 원본 문서 전량 스캔으로 뒷받침해야 하며(위 "각주 서술 전반에 대한 원본성 요건 확장" 참조), "이런 제품군은 보통 이렇다"는 일반화만으로 삭제 근거를 대신하지 않는다.
  - **측정 표준/프로토콜 번들은 원자화하지 않는다 (v1.15 신규)**: AES75처럼 특정 브랜드가 아니라 업계 표준 기구(AES 등)가 발간한 브랜드 무관 측정 프로토콜이라도, 한 번에 여러 하위 수치(예: dBZ/dBZpk/dBA와 그 측정에 쓰인 기준 입력 레벨)를 묶어서 보고하는 방식이라면, 그 하위 수치들을 각각 별도 Key로 쪼개 놓고 다른 브랜드 파일에 4개의 개별 null을 만들지 않는다. 이 표준을 채택하지 않은 브랜드 입장에서는 "이 측정 자체를 제공하지 않는다"는 하나의 사실이지, 4개의 개별 정보 누락이 아니기 때문이다 — 표 순수성 원칙(Key/Value/Unit)의 취지는 유지하되, 실질적으로 하나의 측정 번들인 경우 Value 칸에 복합값을 병기하는 단일 Key로 합친다(예: `AES75_Max_Linear_SPL`의 Value에 "L: 127.5 dBZ / 144.5 dBZpk / 125.5 dBA (input +8.3 dBV)"처럼 베리언트별 전체 번들을 한 셀에 기재). 이 표준을 실제로 채택한 브랜드에서는 그 복합값이 실값으로 채워지고, 미채택 브랜드에서는 그 Key 하나만 null이 된다.
  - **"이름이 다른 브랜드에도 붙을 수 있는가"만으로는 부족하다 — 업계 표준 대역 구분 vs 특정 제품 라인 고유 혁신 구분 (v1.16 신규)**: 위 판별 기준("Key 이름을 다른 브랜드에 붙였을 때 의미가 통하는가")만으로는 놓치는 사례가 있다. L2/L2D에서 신설된 `LC_Transducer`/`LC_Acoustical_Load`/`LC_Nominal_Impedance`(LC = Low frequency Cardioid)를 다른 L-Acoustics 제품(K1, K3 등)과 d&b/Meyer Sound 파일에 null로 동기화했었으나, 이는 잘못된 처리였다 — `LF`/`MF`/`HF`는 음향 업계에서 누구나 쓰는 표준 대역 구분(어느 제조사든 실제로 채택 가능성이 실질적으로 있는 범용 개념)인 반면, `LC`는 L2 제품 라인 하나가 도입한 특정 물리적 설계 혁신(측면 장착 카디오이드 전용 드라이버 그룹)으로, 현재 이 카테고리 전체에서 L2/L2D 단 두 제품에만 존재한다. **판별 기준 보강**: "이 Key가 가리키는 개념이 음향 공학에서 일반적으로 통용되는 표준 대역/스펙 범주(LF/MF/HF, 임피던스, 크로스오버 유무 등)인가, 아니면 특정 제품(군) 하나가 도입한 고유 물리적 설계 요소(카디오이드 전용 대역, GSL의 전방/측면 분리 배선 등)인가?"를 함께 묻는다 — 후자라면 브랜드 간 이름 재사용 가능성과 무관하게 **그 아키텍처를 실제로 가진 제품에만 존재**시키고 다른 파일에는 생성하지 않는다(GSL의 `Front_LF_Transducer`와 동일한 처리). 이 교정에 따라 K1/K2/K3/K3i/Kara_II/Kara_IIi/GSL8/GSL12/PANTHER(총 9개 파일, Kiva_II/KS28은 애초부터 LC Key가 없어 대상 아님)에 잘못 동기화되어 있던 LC_* 3개 Key를 삭제했다(2026-07-17).
  - **Key/컬럼명에 특정 제품명을 origin 흔적으로 남기지 않는다 (v1.18 신규)**: 동적 스키마 발견 과정에서 최초로 그 개념을 도입한 제품의 이름이 Key/컬럼명 자체에 그대로 박혀 다른 제품에도 그대로 복제되는 경우가 있다 — 예: `delay_defaults` 표의 `K1_Delay_ms` 컬럼은 K1이 이 스키마의 최초 origin 제품이었다는 이유만으로 이름이 고정되어, 이후 모든 제품 파일(K2, A10, X8 등)에서도 "그 제품 자신의 1차 엘리먼트 딜레이"를 의미하면서 컬럼명은 계속 `K1_Delay_ms`로 남아 있었다. 이는 브랜드/제품 고유 물리 설계를 가리키는 것이 아니라(위 항목들과는 다른 문제) 단순히 스키마가 진화한 순서의 흔적이 이름에 남은 것으로, 나중에 파일을 보는 사람에게 "왜 A10i 파일에 K1 얘기가 나오지?"라는 혼란을 준다(사용자 지적 2026-07-17). Key/컬럼명은 그 개념을 처음 발견한 제품이 아니라 **그 개념 자체의 범용 역할**을 가리키도록 짓는다 — `K1_Delay_ms` → `Primary_Element_Delay_ms`(1차 엘리먼트 자신의 딜레이, `Secondary_Element_Delay_ms`와 대구를 이루는 이름)로 전면 개명(2026-07-17, 라이브 파일 31개 일괄 정정).
  - **웨이(way) 구조상 존재하지 않는 대역의 Key는 null 동기화하지 않고 삭제한다 (v1.19 신규, v1.14 기존 판단 뒤집음)**: `MF_Transducer`/`MF_Acoustical_Load`/`MF_Nominal_Impedance`/`RMS_Power_Handling_MF`처럼 LF/MF/HF 대역 자체는 범용 개념(v1.16의 "표준 대역 구분")이 맞지만, 확인된 **2-way 제품**(Way_Count=2, MF 대역 자체가 아키텍처상 존재하지 않음)에는 이 4개 Key를 null로 동기화하지 않고 GSL의 `Front_LF_Transducer`와 동일하게 **아예 생성하지 않는다**. 이는 K1 v1.4에서 "2-way/3-way 구분은 범용 개념이므로 MF_Transducer를 null로 동기화 유지한다"고 내렸던 기존 판단을 뒤집는 것이다 — 재검토 결과, "그 대역 구분 개념이 업계 표준인가"와 "이 제품에 그 대역을 담당하는 물리적 드라이버가 있는가"는 별개 질문이며, 후자가 아니라면(2-way 제품의 MF처럼 구조적으로 존재할 수 없다면) null이 아니라 삭제가 맞다(사용자 지적 2026-07-17 — "있지도않은 MF 드라이버에 null 표기를 할 필요가 없다"). **동일 원칙을 LF-only 서브우퍼(Way_Count=1)의 HF_* 4개 Key에도 적용**한다 — 서브우퍼는 애초에 HF 대역 드라이버가 없으므로 `HF_Transducer`/`HF_Acoustical_Load`/`HF_Nominal_Impedance`/`RMS_Power_Handling_HF`도 동일하게 삭제 대상이다. 반면 `Nominal_Impedance_Overall`/`RMS_Power_Handling_Overall`처럼 "단일 채널 패시브·멀티모듈 구조에서 대역별 분리 보고가 불가능해 통합값만 존재"하는 경우는 이 규칙과 무관하다(대역 자체가 없는 게 아니라 개별 보고가 안 되는 것뿐이므로 null 유지). 판별: "이 제품의 실제 way 개수(Way_Count)에 해당 대역이 물리적으로 존재하는가?"를 확인 — 존재하지 않으면 삭제, 존재하지만 수치를 못 찾았으면 null 유지.
</bidirectional_sync_rules>
...
- **동일 내용 반복 수정 작업의 서브에이전트 위임 (v1.19 신규)**: 같은 종류의 수정을 여러 제품 파일에 반복 적용해야 하는 경우(예: 동일한 null-audit 수정 패턴을 N개 제품에 순서대로 적용, 특정 Key를 여러 파일에서 동일하게 삭제/개명하는 작업), 메인 스레드에서 파일마다 순차로 직접 처리하지 않고 서브에이전트에 위임한다. 위임 시 작업의 성격에 따라 모델을 구분한다:
  - **판단이 필요한 반복 작업**(원본 PDF 재조사, 라벨링 오류 발견, 폴라리티/수치 확정 등 소스 문서 해석이 필요한 경우)은 기본(능력 있는) 모델의 서브에이전트에 위임한다.
  - **순수 기계적 반복 작업**(이미 확정된 패턴의 단순 Key 삭제, 단순 리네임처럼 소스 재해석이 필요 없는 경우)은 haiku 모델의 서브에이전트에 위임해 비용을 절감한다.
  - 각 서브에이전트에게는 Surgical Versioning Protocol(Duplicate→Archive→Edit)과 대상 파일 경로, 정확한 변경 내용을 명시적으로 지시한다 — 서브에이전트는 이 대화의 맥락을 모르므로 지침 문서 위치, 판단 기준, 이전 유사 사례(예: X4r의 라벨링 오류 발견 사례)를 프롬프트에 포함한다.
</workspace_versioning_rules>
...
- **동일 계열 임계값 Key의 행 순서(v1.20 신규)**: `Frequency_Response_3dB_Hz`/`_4dB_Hz`/`_5dB_Hz`/`_6dB_Hz`처럼 하나의 물리량을 서로 다른 임계값(dB 등) 기준으로 여러 Key로 나눠 표기하는 경우, 표 안 행 순서는 **발견/추가된 시간순이 아니라 임계값 숫자 내림차순(6 -> 5 -> 4 -> 3)으로 정렬**한다. Key가 여러 세션에 걸쳐 하나씩 추가되면 시간순으로 쌓여 5, 6, 3, 4처럼 뒤섞이기 쉬우므로, 신규 Key를 삽입할 때마다 그 계열 전체의 순서를 이 규칙에 맞게 다시 정렬한다(값 자체는 변경하지 않음, 행 순서만 조정하는 서식 정정이라 Minor 버전).
- **Usable_Bandwidth_Hz의 측정 dB 기준 인라인 표기(v1.21 신규)**: `Usable_Bandwidth_Hz`(및 대역폭 하한/상한을 보고하는 유사 Key)의 `Value` 칸에는 그 대역폭이 몇 dB 감쇠 기준으로 측정된 것인지(-10 dB / -6 dB / -3 dB 등)를 반드시 괄호로 값 바로 뒤에 인라인 표기한다 — 예: `| Usable_Bandwidth_Hz | 35 - 20000 (-10 dB) | Hz |`. dB 임계값은 대역폭 수치의 의미를 좌우하는 필수 조건이므로 대역폭 수치와 분리하면 값 자체가 불완전해진다 — 따라서 이 Key에 한해 dB 기준은 각주에만 두지 않고 값 셀에 함께 표기한다(표 순수성의 "측정 조건은 각주로" 규정에 대한 명시적 예외). 판단 근거는 반드시 그 파일 자신의 원문/각주여야 하며, 원문에 dB 기준이 명시되지 않았거나 각주로 확정되지 않았으면 `(dB 기준 미표기)`를, 조건 의존적 권장 대역이면 `(권장 동작 대역, dB 기준 미표기)`를 붙여 불명확함을 값 셀에서 바로 드러낸다. 원문에 없는 dB 값을 추정해 지어내지 않는다(무결성 원칙). 값이 이미 `null`이면 붙일 대상이 없으므로 그대로 둔다.
</table_purity_rules>
