...
같은 제품명 아래 여러 지향각/폭 변형이 있을 때, 별도 파일로 분리할지 한 파일에 Value 병기(`"L: x / M: y / W: z"`)할지는 공식 문서가 그 변형들을 별개 모델(별도 파트넘버 등)로 취급하는지, 하나의 제품명의 베리언트로 취급하는지에 따라 정한다 — GSL8/GSL12(별도 파트넘버 Z0750/Z0751)는 별도 파일, PANTHER-L/M/W(파트넘버 구분 없이 하나의 Datasheet에 병기)는 한 파일.
