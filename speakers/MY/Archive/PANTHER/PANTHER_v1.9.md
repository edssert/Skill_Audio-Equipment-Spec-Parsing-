...
**Product_Series/Product_Type(신규 Key, 사용자 지시 2026-07-17)**: Product_Type은 OM/AE의 "The self-powered PANTHER™ linear line array loudspeaker..." 서술에서 채택 — L-Acoustics 제품처럼 AE 최상단에 "제품명 - 유형 / 시리즈명" 형식의 고정 헤더가 없어(Meyer Sound 문서 관례가 다름), 본문 서술에서 공식 유형 표현을 확인. Product_Series는 OM/AE/SPS 3개 문서 전량을 "series" 키워드로 스캔했으나 PANTHER를 특정 시리즈에 소속시키는 표현이 전혀 없어(0건 검출) null — 짐작 금지 원칙에 따라 타 브랜드(L-Acoustics/d&b) 관례를 유추 적용하지 않음.
...
**[4] Cardioid_Capability — 4번째 유형(전용 Key 신설, v1.8)**: 원문(SPS) "The PANTHER 2-way loudspeaker has a unique placement of low-mid transducers, and a computationally optimized high horn creating a directional loudspeaker with a controlled pattern for the full frequency range. Arrays of four units and longer functionally create a cardioid polar pattern, which remains consistent at all SPL levels, including when pushed into limiting, and **does not require any user settings**." — 이는 L-Acoustics 계열의 어느 유형과도 다른 4번째 유형이다: CS1/GSL8/GSL12(Built-in, 단일 인클로저 단독 성립)와도, KS28/KS21/SB15m/SB18류(Array-based only, 4대 어레이 중 1대 물리적 반전 필요)와도, L2/L2D(전용 LC 드라이버군+프리셋 선택 필요)와도 다르다 — PANTHER는 전용 카디오이드 드라이버군 없이 표준 LF/MF 드라이버 배치 자체의 기하학적 설계만으로 4대 이상 정상 배치 시 자동으로 카디오이드 패턴이 발현되며, 반전 유닛도 전용 프리셋/사용자 설정도 필요 없다("does not require any user settings"가 원문의 핵심 차별점). 기존(v1.3~v1.7) `Cardioid_Pattern_Array_Min_Size`(=4) Key가 최소 대수만 수치로 담고 있었으나, 이 신규 Key로 "그 카디오이드가 어떻게 성립하는지"의 서술 정보를 보강 — 두 Key 모두 유지(하나는 정성적 유형 분류, 하나는 정량적 최소 대수).
...
**비적용(PANTHER 아키텍처상 개념 자체 불성립, K1/GSL에서 유래)**: Compatible_Amplified_Controller, Max_Enclosures_Per_Controller_Output, Max_Enclosures_Per_Controller_Total, Dimensions_Raw, IP_Rating(physical 섹션, protection_thermal로 이전) — 5건. Input_Connector/Link_Connector/PA_COM_Pinout_*/NLT4_Pinout_*는 PANTHER 자신의 OM/AE/SPS 전량 스캔 결과 PA-COM/NLT4/speakON류 커넥터 언급이 전혀 없어(실제 커넥터는 Analog_Input_Connector/Digital_Input_Connector로 이미 표현됨) 애초에 생성하지 않음(v1.2, 사용자 피드백).

**총계**: null 24건 (미확인 19건 + 비적용 5건). 확정적 비존재(0/No)로 명시한 항목은 1건 — Passive_Crossover_Network(No, 3개 문서 전량 스캔 근거) — 이 수치는 Kiva II/L2 투입 이전 기준. Kiva II/L2 투입으로 신설된 Key 중 Nominal_Impedance_Overall/RMS_Power_Handling_Overall/Max_SPL_Single_Module(3개)이 이 파일에 비적용 null로 추가 반영되어 실제 총계는 위 수치+3건이다. LC_Transducer/LC_Acoustical_Load/LC_Nominal_Impedance는 한때 동기화되어 있었으나 LC가 L2 라인 고유 설계 요소라는 사용자 지적(2026-07-17)에 따라 v1.6에서 삭제(SKILL v1.16). v1.1에서 GSL 고유 배선 구조 명칭 8개 Key 삭제, v1.2에서 스피커 레벨 다채널 커넥터 Key(Input_Connector/Link_Connector/PA_COM_Pinout_*/NLT4_Pinout_*, 7개) 추가 삭제, AES75_Max_Linear_SPL 단일 Key로 통합(실값 보유), Frequency_Response_3dB/6dB_Hz + RMS_Power_Handling_LF/HF null 추가.
...
| v1.6 | LC_Transducer/LC_Acoustical_Load/LC_Nominal_Impedance 삭제(사용자 피드백 2026-07-17) — LC는 L2 라인 고유의 물리적 설계 요소이지 업계 표준 대역 구분이 아니므로 v1.3의 범용 동기화 판단을 정정(SKILL v1.16). |
...
| v1.5 | Max_SPL_4x_Array_Far_Field_Scaled_1m Key가 이전 편집 과정에서 누락되었던 것을 12개 파일 전수 감사로 발견해 복원(null, GSL 유래 범용 측정 조건 — PANTHER 자체 문서는 4대 어레이 SPL을 보고하지 않음). |
| v1.7 | delay_defaults 표의 컬럼명 `K1_Delay_ms`를 `Primary_Element_Delay_ms`로 전면 개명(SKILL v1.18) — 값 변경 없음(사용자 지적 2026-07-17). |
| v1.8 | MF_Transducer/MF_Acoustical_Load/MF_Nominal_Impedance/RMS_Power_Handling_MF 4개 Key 삭제(SKILL v1.19) — 이 제품은 확인된 2-way 구조라 MF 대역 드라이버 자체가 없으므로, null 동기화 대상이 아니라 삭제 대상으로 재분류(K1 v1.4의 기존 반대 판단을 뒤집는 정책 변경, GSL의 Front_LF_Transducer와 동일 원칙 적용). |
