...
**미확인(정보 부족)**: Model_Number, WEEE_Marking, Nominal_Directivity_Vertical, DSP_Preset_Name, LF_Acoustical_Load, AES67_Support, Service_Port_Type, Handles(정확한 개수), Rigging_Components_Material, Finish_Type, Max_Wind_Load, preset_guide_and_matching(전 항목), delay_defaults(전 항목), mechanical_safety(Safe_Limit 열, 기본 Grid Kit 단독 구성 상한) — 19건.
**비적용(PANTHER 아키텍처상 개념 자체 불성립, K1/GSL에서 유래)**: MF_Transducer, MF_Acoustical_Load, MF_Nominal_Impedance, Input_Connector, Link_Connector, PA_COM_Pinout_AB, PA_COM_Pinout_CD, PA_COM_Pinout_EF, PA_COM_Pinout_GH, NLT4_Pinout_Pin1, NLT4_Pinout_Pin2, Compatible_Amplified_Controller, Max_Enclosures_Per_Controller_Output, Max_Enclosures_Per_Controller_Total, Dimensions_Raw, IP_Rating(physical 섹션, protection_thermal로 이전) — 16건.

**총계**: null 35건 (미확인 19건 + 비적용 16건). 확정적 비존재(0/No)로 명시한 항목은 1건 — Passive_Crossover_Network(No, 3개 문서 전량 스캔 근거). v1.1에서 Front_LF_Transducer/Side_LF_Transducer/Front_LF_Nominal_Impedance/Side_LF_MF_HF_Nominal_Impedance/Power_Handling_Front_LF_RMS/Power_Handling_Front_LF_Peak_10ms/Power_Handling_Side_LF_MF_HF_RMS/Power_Handling_Side_LF_MF_HF_Peak_10ms(8개, GSL 고유 배선 구조 명칭) 삭제.
