# PANTHER-M (Meyer Sound) — Master Schema
...
| Key | Value | Unit |
|---|---|---|
| Product_Name | PANTHER-M | - |
| Model_Number | null | - |
| Product_Series | null | - |
| Product_Category | Line Array | - |
| Product_Type | Self-powered line array loudspeaker | - |
| Description | self-powered, linear, low-distortion line array loudspeaker, 2-way, 95° horizontal coverage | - |
| Way_Count | 2 | - |
...
**Product_Name="PANTHER-M"**: 원문(SPS/OM) 자체가 "PANTHER-L"/"PANTHER-M"/"PANTHER-W" 표기를 사용 — GSL8/GSL12 전례와 동일하게 원문 모델 표기를 Product_Name으로 채택.
...
| AES75_Max_Linear_SPL [2] | 126.5 dBZ / 144 dBZpk / 125 dBA (input +6.8 dBV) | - |
| Linear_Peak_SPL | null | - |
| Nominal_Directivity_Horizontal_deg | 95 | deg |
...
**출처**: SPS p.1-2 "Specifications" 표(ACOUSTICAL, PANTHER-M 열); AE p.1; OM p.1-2.
**[1] Max_SPL_Peak 측정 조건**: SPS 각주 "Maximum Sound Level is the Lpeak measured using burst noise", AE는 "measured in free-field at 4 m referred to 1 m using noise"로 더 상세 — 두 문서 값 일치.
...
**총계**: null 37건 (미확인 29건 + 비적용 8건). 확정적 비존재(0/No)로 명시한 항목은 1건 — Passive_Crossover_Network(No, 3개 문서 전량 스캔 근거). **추가 정정(2026-07-18, 같은 감사 라운드)**: mechanical_safety Safe_Limit 결합형 불릿 오카운트(2개 행을 1건으로 카운트) 정정 — 미확인 28건→29건, 총계 36건→37건. 데이터(Key/Value/Unit) 자체는 전혀 변경하지 않았다.

## 버전 변경 이력

| Version | 변경 내용 |
|---|---|
| v1.0 | PANTHER-L/M/W 개별 파일 분리(사용자 지시 2026-07-18) — 통합 파일 `PANTHER_v1.9.md`(v1.0~v1.9 전체 이력, `speakers/MY/Archive/PANTHER/`에 보존)에서 PANTHER-M 베리언트만 분리해 이 파일로 시작. 베리언트별 값(Max_SPL_Peak=150.5dB, AES75_Max_Linear_SPL, Nominal_Directivity_Horizontal_deg=95°)만 단일화하고 나머지는 통합 파일의 최종 상태(v1.9) 그대로 승계. |
