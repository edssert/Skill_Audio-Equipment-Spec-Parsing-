...
| Key | Value | Unit |
|---|---|---|
| Operating_Frequency_Range_Hz | 55 - 16000 | Hz |
| Max_SPL_Peak [1] | L: 150.5 / M: 150.5 / W: 149.5 | dB |
| AES75_Max_Linear_SPL_dBZ [2] | L: 127.5 / M: 126.5 / W: 126.5 | dBZ |
| AES75_Max_Linear_SPL_dBZpk [2] | L: 144.5 / M: 144 / W: 142.5 | dBZpk |
| AES75_Max_Linear_SPL_dBA [2] | L: 125.5 / M: 125 / W: 123 | dBA |
| AES75_Reference_Input_Level_dBV [2] | L: 8.3 / M: 6.8 / W: 6.4 | dBV |
...
**[2] AES75_* 측정 조건**: AES75-2022 표준 기준, 자유음장 4m 측정 후 1m 환산, 각 베리언트별 RMS 입력 레벨(AES75_Reference_Input_Level_dBV)이 다름(L=8.3dBV, M=6.8dBV, W=6.4dBV) — 이 입력 레벨 자체가 측정 조건의 일부이므로 별도 Key로 보존.
...
**MF_* 계열 null 사유(비적용)**: PANTHER는 2-way(LF+HF)이며 MF 대역이 없음 — K1(3-way)에서 유래한 Key라 비적용.
**Front/Side_LF_* 계열 null 사유(비적용)**: GSL에서 유래한 Key. PANTHER의 LF 드라이버 2발은 GSL처럼 전방/측면 분리 배치가 아니라 동일 그룹으로 판단(SPS/AE에 별도 위치 구분 서술 없음) — 비적용.
**Power_Handling_* null 사유(미확인)**: GSL에서 유래한 Key. PANTHER 3개 문서 전량 스캔했으나 "power handling"/"RMS" 대역별 파워 정격 표기를 찾지 못함(SPS는 시스템 레벨 Max Long-Term Continuous Power/Burst Power만 제공, 이는 power_supply 섹션에 별도 기재) — 미확인.
...
**Input_Connector/Link_Connector/PA_COM_Pinout_*/NLT4_Pinout_* 비적용 사유**: K1/GSL에서 유래한 다핀 통합 커넥터(PA-COM, NLT4) 개념이 PANTHER에는 없음 — PANTHER는 별도 아날로그(XLR)/디지털(etherCON) 입력을 사용.
...
**미확인(정보 부족)**: Model_Number, WEEE_Marking, Nominal_Directivity_Vertical, DSP_Preset_Name, LF_Acoustical_Load, Power_Handling_Front_LF_RMS, Power_Handling_Front_LF_Peak_10ms, Power_Handling_Side_LF_MF_HF_RMS, Power_Handling_Side_LF_MF_HF_Peak_10ms, AES67_Support, Service_Port_Type, Handles(정확한 개수), Rigging_Components_Material, Finish_Type, Max_Wind_Load, preset_guide_and_matching(전 항목), delay_defaults(전 항목), mechanical_safety(Safe_Limit 열, 기본 Grid Kit 단독 구성 상한) — 23건.
**비적용(PANTHER 아키텍처상 개념 자체 불성립, K1/GSL에서 유래)**: MF_Transducer, MF_Acoustical_Load, MF_Nominal_Impedance, Front_LF_Transducer, Side_LF_Transducer, Front_LF_Nominal_Impedance, Side_LF_MF_HF_Nominal_Impedance, Input_Connector, Link_Connector, PA_COM_Pinout_AB, PA_COM_Pinout_CD, PA_COM_Pinout_EF, PA_COM_Pinout_GH, NLT4_Pinout_Pin1, NLT4_Pinout_Pin2, Compatible_Amplified_Controller, Max_Enclosures_Per_Controller_Output, Max_Enclosures_Per_Controller_Total, Dimensions_Raw, IP_Rating(physical 섹션, protection_thermal로 이전) — 20건.

**총계**: null 43건 (미확인 23건 + 비적용 20건). 확정적 비존재(0/No)로 명시한 항목은 1건 — Passive_Crossover_Network(No, 3개 문서 전량 스캔 근거).
