...
**출처**: A10i_AE_EN.docx "Physical data" - "Dimensions (W, H, D): 569 mm, 347 mm, 345 mm".
**Dimensions_Raw 미확인**: OM 치수 도면 세부 수치는 시간 배분상 별도 추출하지 않음(AE 확정 W/H/D로 충분).
...
**출처**: A10i_OM_EN.pdf "Loudspeaker configurations"(A10i Focus와 동일 구조 공유, KS21i 조합 상세는 시간 배분상 미확인).
...
**출처**: A10i_OM_EN.pdf "Mechanical safety"(A10i Focus와 공용). 상세 표는 시간 배분상 미확인.

## Null Report

**미확인(정보 부족)**: Model_Number, WEEE_Marking, Frequency_Response_5dB_Hz, Max_SPL_4x_Array_Far_Field_Scaled_1m, AES75_Max_Linear_SPL, LF_Nominal_Impedance, MF_Nominal_Impedance, HF_Nominal_Impedance(단일 통합값만 존재), RMS_Power_Handling_LF/MF/HF(단일 통합값만 존재), Max_Enclosures_Per_Controller_Output, Max_Enclosures_Per_Controller_Total, Handles, Dimensions_Raw, Rigging_Components_Material, Max_Wind_Load, preset_guide_and_matching(KS21i 조합, Recommended_Ratio/Minimum_Line_Length), delay_defaults(전 항목), mechanical_safety(전 항목) — 19건.
...
**총계**: null 24건 (미확인 19건 + 비적용 5건). 확정적 비존재(0/No)로 명시한 항목은 0건.
