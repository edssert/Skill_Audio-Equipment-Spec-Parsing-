...
**비적용**: 없음 — 0건. (PA_COM_Pinout_*/SpeakON_Pinout_* 등 타 커넥터 고유 Key는 SB18 IIi 자신의 Terminal_Block_Pinout_*으로 이미 표현되어 애초에 생성하지 않음, null 목록에서 제외. MF/HF 관련 8개 Key는 LF 전용 서브우퍼 구조상 애초에 생성하지 않음, SKILL v1.19.)

**총계**: null 16건 (미확인 16건 + 비적용 0건). 확정적 비존재(0/No)로 명시한 항목은 2건 — Handles(0, OM+AE 전량 스캔 "handle" 0건 검출), Passive_Crossover_Network(No, 전량 스캔 "crossover" 0건 검출).
