...
**참고**: 이 텍스트 자체에 포함된 출처 각주(예: "preset_guide_EN.pdf p.49-50 owner's manual EN v29.0", "manualslib.com 게재본 v6.0")는 웹 페이지가 자체적으로 표기한 출처이며, 이 프로젝트의 `upload/`에는 K1_OM_EN.pdf(version 4.0)만 제공되어 있다 — preset_guide_EN.pdf나 v29.0/v6.0 매뉴얼은 이 세션에 실물로 제공되지 않았다. 마스터 스키마 반영 시 이 차이를 명시하고, 실제 원본이 없는 인용은 "웹 소스 자체 표기"로만 신뢰도를 제한한다.
