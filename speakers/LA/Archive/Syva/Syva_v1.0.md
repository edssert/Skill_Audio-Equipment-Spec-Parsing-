...
**미확인(정보 부족)**: Model_Number, Frequency_Response_5dB_Hz, Max_SPL_4x_Array_Far_Field_Scaled_1m, AES75_Max_Linear_SPL, LF_Transducer, LF_Acoustical_Load, LF_Nominal_Impedance, MF_Nominal_Impedance, HF_Nominal_Impedance, RMS_Power_Handling_LF/MF/HF, Handles, Inter_Enclosure_Angles_deg, Rigging_Components_Material, Max_Wind_Load, preset_guide_and_matching(Recommended_Ratio 5행, Minimum_Line_Length 전 행) — 18건.
**비적용**: CUT_Mode_Available, HFC_Function_Settings, Coupling_Function_Range, Link_Connector(AutoConnect가 그 역할을 대신함) — 4건. (PA_COM_Pinout_*/Terminal_Block_Pinout_* 등 타 커넥터 고유 Key는 Syva 자신의 SpeakON_Pinout_*/Screw_Terminal_Pinout_*/AutoConnect_Pinout으로 이미 표현되어 애초에 생성하지 않음, null 목록에서 제외.)

**총계**: null 22건 (미확인 18건 + 비적용 4건). 확정적 비존재(0/No)로 명시한 항목은 1건 — Cardioid_Capability(No, 원문 전량 스캔 근거).
