...
**미확인(정보 부족)**: Model_Number, Frequency_Response_5dB_Hz, Max_SPL_4x_Array_Far_Field_Scaled_1m, AES75_Max_Linear_SPL, Nominal_Directivity_Horizontal_deg, RMS_Power_Handling_LF, Max_Enclosures_Per_Controller_Output/Total(LA2Xi BTL 모드 값), Handles, Rigging_Components_Material, Max_Wind_Load, preset_guide_and_matching(Recommended_Ratio/Minimum_Line_Length) — 12건.
**비적용**: CUT_Mode_Available, HFC_Function_Settings, Coupling_Function_Range, Link_Connector, Inter_Enclosure_Angles_deg — 5건. (PA_COM_Pinout_*/Terminal_Block_Pinout_* 등 타 커넥터 고유 Key는 Syva Low 자신의 SpeakON_Pinout_*/AutoConnect_Pinout으로 이미 표현되어 애초에 생성하지 않음, null 목록에서 제외.)

**총계**: null 17건 (미확인 12건 + 비적용 5건). 확정적 비존재(0/No)로 명시한 항목은 2건 — Passive_Crossover_Network(No, 원문 전량 스캔 근거), Cardioid_Capability(No, 원문 전량 스캔 근거).
