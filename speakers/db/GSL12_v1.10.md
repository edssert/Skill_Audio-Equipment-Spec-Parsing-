...
**Product_Series/Product_Type(신규 Key, 사용자 지시 2026-07-17)**: GSL8과 동일 근거(공용 OM, "SL-Series" 명시적 확인) — 상세는 GSL8_v1.8.md 참조.

## acoustical_performance

| Key | Value | Unit |
|---|---|---|
| Usable_Bandwidth_Hz [1] | 41 - 20000 (-10 dB) | Hz |
...
| Nominal_Directivity_Horizontal_deg [3] | 120 constant directivity (merging into cardioid dispersion below 150 Hz) | deg |
...
**Frequency_Response_4dB_Hz(신규 Key)**: GSL8과 동일 사유 — 미확인.
**[3] 저음역 카디오이드 수렴 특성**: GSL8과 동일 원문 근거(OM p.9, GSL8/GSL12 공용 챕터) — "120°가 150Hz 이상에서 유지되다가 그 이하에서 카디오이드 형태로 수렴"(사용자 지적으로 2026-07-17 재조사, 상세 근거는 GSL8_v1.7.md 참조).
**[4] Cardioid_Capability=Built-in(전용 Key 신설, v1.7)**: GSL8과 동일 원문·동일 유형(GSL8/GSL12 공용 챕터, 단일 인클로저만으로 성립하는 하이브리드 크로스오버 기반 저역 카디오이드 수렴) — 상세 근거는 GSL8_v1.7.md acoustical_performance 각주 [4] 참조.
...
**출처**: OM Chapter 2.1 p.6, Chapter 2.5 p.11 "GSL8/GSL12 cabinet dimensions in mm [inch]"(GSL8/GSL12 공용 도면, 두 모델이 함께 표기됨 — 캐비닛 외형 치수 동일).
**[1] Width_mm/Height_mm/Depth_mm/Dimensions_Raw 재조사 완료(2026-07-18)**: GSL8과 동일 근거 — PowerShell+Windows.Data.Pdf 이미지 렌더링으로 OM p.11 도면 육안 확인, Width=1300mm/Height=391mm/Depth=627mm(GSL8/GSL12 공용 도면이므로 동일값). 상세 판단 근거는 GSL8_v1.8.md 참조.
**IP_Rating/Rigging_Components_Material/Finish_Color 미확인 사유**: GSL8과 동일 — 상세는 GSL8_v1.8.md 참조.
...
**실값 확보(v1.9, 2026-07-18)**: GSL8_v1.9.md와 동일 — GSL/KSL 공용 Rigging manual(신규 확보) Chapter 1.3/1.5 근거. 캐비닛 중량(80kg) x 24대 = 1920kg로 2000kg SWL 이내(GSL12도 GSL8과 동일 중량).
**Safety_Factor**: GSL8과 동일 사유로 미확인.

## Null Report

**미확인(정보 부족)**: Nominal_Directivity_Vertical, Frequency_Response_6dB_Hz, Frequency_Response_3dB_Hz, Frequency_Response_4dB_Hz, AES75_Max_Linear_SPL, Max_Enclosures_Per_Controller_Total, LF_Acoustical_Load, RMS_Power_Handling_LF, RMS_Power_Handling_MF, RMS_Power_Handling_HF, Rigging_Components_Material, Finish_Color, IP_Rating, mechanical_safety(Safe_Limit 열 — 3개 Configuration 행의 null 셀, 3건; Safety_Factor, 1건) — 17건.
**비적용(GSL 아키텍처상 개념 자체 불성립)**: DSP_Preset_Name, LF_Transducer, LF_Nominal_Impedance(GSL은 Front_LF_Nominal_Impedance/Side_LF_MF_HF_Nominal_Impedance로 이미 자체 표현, MF/HF_Nominal_Impedance와 동일 사유), MF_Nominal_Impedance, HF_Nominal_Impedance, Link_Connector, HFA_Function_Settings(point-source setup이 없는 구조), Peak_Power_Handling_10ms_Overall(2채널 하이브리드 구조, front/side 분리 보고), preset_guide_and_matching(전 항목), delay_defaults(전 항목) — 10건. PA_COM_Pinout_*는 L-Acoustics 커넥터 고유 명칭이라 애초에 생성하지 않음.

**총계**: null 27건 (미확인 17건 + 비적용 10건). 확정적 비존재(0/No)로 명시한 항목은 0건 — Passive_Crossover_Network는 "Yes"로 실값 확보. LC_Transducer/LC_Acoustical_Load/LC_Nominal_Impedance는 한때 동기화되어 있었으나 LC가 L2 라인 고유 설계 요소라는 사용자 지적(2026-07-17)에 따라 v1.5에서 삭제(SKILL v1.16). (v1.4: LF_Nominal_Impedance 누락 추가, 총계 산술 정정.) **v1.9(2026-07-18)**: GSL/KSL Rigging manual 신규 확보로 mechanical_safety 대부분 실값 전환(총계는 24건 동일, 구성만 재분배). **v1.10(2026-07-19)**: speakers 카테고리 전체 일괄 동기화 라운드 — 신규 Key 3종(Frequency_Response_4dB_Hz=미확인, HFA_Function_Settings=비적용, Peak_Power_Handling_10ms_Overall=비적용) 반영, 총계 24건→27건. GSL8과 총계까지 일치.
...
| v1.7 | Nominal_Directivity_Horizontal_deg에 저음역 카디오이드 수렴 특성 추가(사용자 지적 2026-07-17) — OM Chapter 2.4 "Dispersion characteristics" p.9 원문(GSL8과 공용 챕터)을 재조사로 발견, Value에 병기. |
| v1.8 | GSL8_v1.8.md와 동일 — Cardioid_Capability 전용 Key 신설(Built-in), Product_Series="SL-Series"/Product_Type="Line array loudspeaker" 신규 Key 적용, Width_mm/Height_mm/Depth_mm/Dimensions_Raw 재조사(1300/391/627mm, GSL8과 공용 도면). Null Report 20건(미확인)→16건으로 감소. |
| v1.8 to v1.9 | (양방향 동기화, Minor) GSL8_v1.9.md와 동일 — KSL8/KSL12 투입 과정에서 GSL/KSL 공용 Rigging manual 신규 확보, mechanical_safety Maximum_Limit 3개 조건(10대/800kg, 24대/2000kg SWL, SUB 컬럼 14대) 및 Max_Wind_Load(6/8 Beaufort) 실값 반영. Null Report 총계는 24건으로 동일. |
| v1.10 | speakers 카테고리 전체 일괄 동기화 라운드(2026-07-19) — 신규 Key 3종 반영: `Frequency_Response_4dB_Hz`(미확인), `HFA_Function_Settings`(비적용), `Peak_Power_Handling_10ms_Overall`(비적용). Null Report 24건→27건(미확인 17+비적용 10). GSL8_v1.10.md와 동일 처리. |
