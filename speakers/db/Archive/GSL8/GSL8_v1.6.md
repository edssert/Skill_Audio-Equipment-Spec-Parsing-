...
| Nominal_Directivity_Horizontal_deg | 80 constant directivity | deg |
...
**[1] Dimensions_Raw 미확인 사유**: 치수 도면이 순수 그래픽으로만 표기되어 있고(텍스트 레이어에 치수 숫자가 전혀 없음, pdftotext 전량 추출로 확인) 이 프로젝트 환경에 페이지 이미지 렌더링 도구(pdftoppm)가 설치되어 있지 않아 시각 확인이 불가능했다. K1과 달리 raw 텍스트에도 숫자가 전혀 없어 완전 미확인(K1은 최소한 축 미확정 숫자라도 확보됨).
...
**미확인 사유**: GSL8/GSL12 OM은 "A detailed description of the SL-Series rigging components is given in the SL-Series Rigging manual"이라 언급할 뿐, 플라잉/스태킹 엘리먼트 수 안전 한계표 자체는 본 OM에 포함되어 있지 않다 — 별도 문서(SL-Series Rigging manual) 미제공으로 미확인 처리(비적용이 아님 — 실제로 안전 한계가 존재하지 않는다는 뜻이 아니라 이 문서 범위에 없다는 의미이므로).
...
**Safety_Factor/Max_Wind_Load(K1/PANTHER 투입으로 신설, 미확인, v1.1)**: GSL8/GSL12 OM에는 안전계수나 풍하중 한계에 대한 구체적 수치가 없음(ArrayCalc 소프트웨어로 검증 권장한다는 서술만 있음, 위 rigging_handling 섹션 참조) — 미확인.

## Null Report

**미확인(정보 부족)**: Nominal_Directivity_Vertical, Frequency_Response_6dB_Hz, Frequency_Response_3dB_Hz, AES75_Max_Linear_SPL, Max_Enclosures_Per_Controller_Total, LF_Acoustical_Load, RMS_Power_Handling_LF, RMS_Power_Handling_MF, RMS_Power_Handling_HF(K1/K2 고유 LF/MF/HF 대역 구조로는 매핑 안 됨, GSL 자체 Power_Handling_Front_LF/Side_LF_MF_HF 참조), Width_mm, Height_mm, Depth_mm, Dimensions_Raw, Rigging_Components_Material, Finish_Color, IP_Rating, mechanical_safety(Configuration/Rigging_Accessory/Safe_Limit/Maximum_Limit, Safety_Factor, Max_Wind_Load) — 20건.
...
**총계**: null 28건 (미확인 20건 + 비적용 8건). 확정적 비존재(0/No)로 명시한 항목은 0건 — Passive_Crossover_Network는 "Yes"로 실값 확보(GSL 자신의 OM에 명시) — 이 수치는 Kiva II/L2 투입 이전 기준. Kiva II/L2 투입으로 신설된 Key 중 Nominal_Impedance_Overall/RMS_Power_Handling_Overall/Max_SPL_Single_Module(3개)이 이 파일에 비적용 null로 추가 반영되어 실제 총계는 위 수치+3건이다. LC_Transducer/LC_Acoustical_Load/LC_Nominal_Impedance는 한때 동기화되어 있었으나 LC가 L2 라인 고유 설계 요소라는 사용자 지적(2026-07-17)에 따라 v1.5에서 삭제(SKILL v1.16). (v1.4: 전수 감사에서 두 가지 오류 발견 및 수정 — (1) 표에 null로 존재하는 LF_Nominal_Impedance가 미확인/비적용 목록 어디에도 누락되어 있었음, MF/HF_Nominal_Impedance와 동일 사유(GSL 자체 Front/Side 분리 구조)로 비적용에 추가. (2) 총계 줄의 미확인/비적용 숫자(13건/14건)가 각 목록 자체의 항목 수(20건/7건, LF_Nominal_Impedance 추가 후 8건)와 불일치하던 것을 정정, 총계를 28건(20+8)으로 재계산.)
