...
| Key | Value | Unit |
|---|---|---|
| Usable_Bandwidth_Hz [1] | 41 - 20000 (-10 dB, IEC60268) | Hz |
...
**Peak_Power_Handling_10ms_Overall(신규 Key)**: d&b CCL8에서 신설된 범용 Key(10ms 피크 파워 핸들링) — RMS_Power_Handling_Overall과 동일 사유(2채널 하이브리드 구조, front/side 분리 보고)로 비적용.
...
| v1.10 | speakers 카테고리 전체 일괄 동기화 라운드(2026-07-19, T10/V-Series 등 db 대량 배치 완료 후) — 신규 Key 3종 반영: `Frequency_Response_4dB_Hz`(Meyer Sound 배치 유래, 미확인), `HFA_Function_Settings`(T10 유래, point-source setup 없는 구조라 비적용), `Peak_Power_Handling_10ms_Overall`(CCL8 유래, 2채널 하이브리드 구조상 Overall 통합 보고 불가로 비적용). Null Report 24건→27건(미확인 17+비적용 10). |
